...
describe("BTC/USDT:USDT限定設定テスト", () => {
  ...
  test("1分足がサポートされていないこと", () => {
    ...
    const oneMinute = SUPPORTED_TIMEFRAMES.find(
      (tf) => tf.value === ("1m" as any)
    );
  ...
  test("BTC現物取引ペアがサポートされていないこと", () => {
    ...
    const spotPair = SUPPORTED_TRADING_PAIRS.find(
  ...
  test("BTCUSD無期限先物がサポートされていないこと", () => {
    // BTCUSDはサポートされていないはず
    const usdPair = SUPPORTED_TRADING_PAIRS.find(
      (pair) => pair.symbol === "BTCUSD"
    );
  ...
  test("ETH関連の取引ペアがサポートされていないこと", () => {
    ...
    const ethPairs = SUPPORTED_TRADING_PAIRS.filter(
      (pair) => pair.symbol.includes("ETH") || pair.base === "ETH"
    );
    expect(ethPairs).toHaveLength(0);
  });

  test("BTC/USDT:USDTペアの詳細情報が正しいこと", () => {
    const btcUsdtPerp = SUPPORTED_TRADING_PAIRS.find(
      (pair) => pair.symbol === "BTC/USDT:USDT"
    );
