...
describe("BTC/USDT:USDT限定APIエンドポイントテスト", () => {
  ...
  describe("OHLCVデータ収集API", () => {
    test("デフォルトシンボルがBTC/USDT:USDTであること", async () => {
      ...
      const request = new NextRequest(
        "http://localhost:3000/api/data/ohlcv/collect",
        {
          method: "POST",
        }
      );
  ...
  describe("Funding Rate収集API", () => {
    test("デフォルトシンボルがBTC/USDT:USDTであること", async () => {
      ...
      const { POST } = await import(
        "@/app/api/data/funding-rates/collect/route"
      );

      const request = new NextRequest(
        "http://localhost:3000/api/data/funding-rates/collect",
        {
          method: "POST",
        }
      );
  ...
  describe("Open Interest収集API", () => {
    test("デフォルトシンボルがBTC/USDT:USDTであること", async () => {
      ...
      const { POST } = await import(
        "@/app/api/data/open-interest/collect/route"
      );

      const request = new NextRequest(
        "http://localhost:3000/api/data/open-interest/collect",
        {
          method: "POST",
        }
      );
  ...
  describe("一括収集API", () => {
    test("一括収集がBTC/USDT:USDTのみを対象とすること", async () => {
      ...
      const request = new NextRequest(
        "http://localhost:3000/api/data/all/bulk-collect",
        {
          method: "POST",
        }
      );
  ...
  describe("シンボル一覧API", () => {
    test("シンボル一覧がBTC/USDT:USDTのみを返すこと", async () => {
      ...
      const request = new NextRequest(
        "http://localhost:3000/api/data/symbols",
        {
          method: "GET",
        }
      );
