...
const ParameterEditor: React.FC<ParameterEditorProps> = ({
  ...
  const [indicatorDefinitions, setIndicatorDefinitions] = useState<
    Record<string, IndicatorParameterInfo>
  >({});
    ...
    const fetchIndicatorDefinitions = async () => {
      try {
        ...
        if (data.success) {
          ...
          // カテゴリ別の指標を統合
          (Object.values(data.data.categories) as any[][]).forEach(
            (categoryIndicators: any[]) => {
  ...
  const validateParameter = (
    ...
    const paramDef = definition.parameters.find(
    ...
    const errors = validationErrors.filter(
      (err) =>
        !(
          err.indicatorId === indicatorId && err.parameterName === parameterName
        )
    );
  ...
  const renderParameterField = (
    ...
    const currentValue =
      indicator.parameters[paramDef.name] ?? paramDef.default;
    ...
    if (paramDef.options) {
      ...
      return (
        <SelectField
          ...
          onChange={(value) =>
            handleParameterChange(indicator.id, paramDef.name, value)
          }
              ...
              className={`
                ...
                ${
                  indicator.enabled
                    ...
                    : "border-gray-700 bg-secondary-950 opacity-60"
                }
              `}
            >
              {/* 指標ヘッダー */}
                ...
                <div className="flex items-center gap-3">
                  <label className="flex items-center gap-2 cursor-pointer">
                    <input
                      ...
                      onChange={(e) =>
                        onIndicatorToggle(indicator.id, e.target.checked)
                      }
          ...
          <div>
            ...
            <p
              className={`font-medium ${
                validationErrors.length > 0 ? "text-red-400" : "text-green-400"
              }`}
            >
