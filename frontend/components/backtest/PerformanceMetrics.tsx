...
function MetricCard({ title, value, subtitle, color = 'gray', icon }: MetricCardProps) {
  const colorClasses = {
...
export default function PerformanceMetrics({ result }: PerformanceMetricsProps) {
  ...
  const getReturnColor = (value: number): 'green' | 'red' | 'gray' => {
    if (value > 0) return 'green';
    if (value < 0) return 'red';
    return 'gray';
  };

  const getSharpeColor = (value: number): 'green' | 'yellow' | 'red' | 'gray' => {
  ...
  const finalEquity = result.initial_capital * (1 + metrics.total_return);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-gray-700/30 rounded-lg p-4">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
          <div>
            ...
            <span className="ml-2 text-white font-medium">{result.strategy_name}</span>
          </div>
          <div>
          ...
          <div>
            ...
            <span className="ml-2 text-white font-medium">{result.timeframe}</span>
          </div>
          <div>
      ...
      {/* 主要パフォーマンス指標 */}
      <div>
        <h3 className="text-lg font-semibold mb-3 text-white">パフォーマンス指標</h3>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <MetricCard
            ...
            subtitle={`${metrics.winning_trades}勝 / ${metrics.losing_trades}敗`}
      ...
      {/* 期間情報 */}
      <div className="bg-gray-700/30 rounded-lg p-4">
        <h3 className="text-lg font-semibold mb-3 text-white">バックテスト期間</h3>
