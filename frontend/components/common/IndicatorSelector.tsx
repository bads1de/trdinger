...
const IndicatorSelector: React.FC<IndicatorSelectorProps> = ({
  ...
  const isLoading =
    indicatorsLoading || (showCategories && categoriesLoading) || infoLoading;
          ...
          Object.entries(categories).map(([category, categoryIndicators]) => (
            <div key={category} className="space-y-2">
              <h4 className="font-medium text-gray-700 capitalize">
                {getCategoryDisplayName(category)} ({categoryIndicators.length}
                個)
              </h4>
              <div className="grid grid-cols-3 gap-2 pl-4">
                {categoryIndicators.map((indicator: any) => (
                  <IndicatorCheckbox
                    key={indicator}
                    indicator={indicator}
...
export const useIndicatorSelection = (initialIndicators: string[] = []) => {
  const [selectedIndicators, setSelectedIndicators] =
    React.useState<string[]>(initialIndicators);
