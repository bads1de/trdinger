...
interface CompactDataCollectionButtonsProps {
  ...
  onFundingRateCollectionStart?: (result: BulkFundingRateCollectionResult | FundingRateCollectionResult) => void;
  onFundingRateCollectionError?: (error: string) => void;
  disabled?: boolean;
}

const CompactDataCollectionButtons: React.FC<CompactDataCollectionButtonsProps> = ({
  ...
  const handleFundingRateCollection = async () => {
    if (!confirm("ファンディングレートデータを収集しますか？")) return;
    ...
    try {
      ...
      if (response.ok && result.success) {
        ...
        onFundingRateCollectionError?.(result.message || "ファンディングレート収集に失敗しました");
      }
    } catch (error) {
      onFundingRateCollectionError?.("ファンディングレート収集中にエラーが発生しました");
    } finally {
