...
現在、指標の命名ロジックが複数のファイルに分散しており、特にレガシー形式と JSON 形式の名前生成が混在しています。これにより、一貫性が損なわれ、混乱を招く可能性があります。

- `backend/app/core/services/auto_strategy/factories/indicator_calculator.py`:
  - `_generate_indicator_name` メソッドが JSON 形式の指標名を生成しようとしていますが、そのロジックは単純に `indicator_type` を返しています。複合指標の結果処理 (`_handle_complex_result`) でも命名ロジックが呼ばれています。
- `backend/app/core/services/auto_strategy/factories/indicator_initializer.py`:
  - `_get_legacy_indicator_name` メソッドが存在し、レガシー形式の指標名を生成しています。これは `indicator_registry` を参照しつつも、手動での文字列フォーマットを行っています。
  - `_get_final_indicator_name` は非推奨とコメントされていますが、コードは残っています。
- `backend/app/core/services/indicators/config/indicator_config.py`:
  - `IndicatorConfig` クラスが `generate_json_name` と `generate_legacy_name` を持っており、本来の命名ロジックの定義元となるべき場所です。
- `backend/app/core/services/indicators/adapters/base_adapter.py`:
  - `_generate_indicator_name` と `_generate_legacy_name` という静的メソッドが存在し、ここでも命名ロジックが重複しています。

これらの分散と重複により、以下の問題が発生しています。

- **一貫性の欠如**: 指標名の生成方法が明確でなく、特にレガシー形式と JSON 形式の切り替えや共存が複雑になっています。
- **保守性の低下**: 命名規則の変更や新しい指標の追加時に、複数の場所を修正する必要があり、エラーの温床となります。
- **冗長なコード**: 複数のファイルに同様の命名ロジックが存在し、DRY 原則に反しています。
...
OI/FR データソースのハードコードされたフォールバック値の使用を見直し、より堅牢で明確なデータハンドリング戦略を導入することを提案します。

**変更案の概要:**

1.  **データ不足時の明確なハンドリング**:
    - ハードコードされたフォールバック値を使用する代わりに、データが利用できない場合に `None` や `NaN` を返す、または明示的なエラーを発生させるようにします。
    - これにより、データが欠落していることが計算の早い段階で明確になり、その後のロジックで適切な対応を取れるようになります。
2.  **データ補間または戦略的スキップの導入**:
    - データが一時的に欠落している場合、単純なフォールバック値ではなく、前後のデータに基づいた補間（例: 線形補間、最後の既知の値を転用）を検討します。ただし、これは戦略の性質とデータの特性に依存するため、慎重な検討が必要です。
    - または、データが不足している期間については、その期間の取引をスキップする、または戦略の評価から除外するなどの戦略的な判断を組み込みます。
3.  **データソースの信頼性向上**:
    - 根本的な解決策として、OI/FR データの収集プロセスとストレージの信頼性を高めることを検討します。例えば、定期的なデータ整合性チェック、データ再取得メカニズムの強化などです。
4.  **設定によるフォールバック値の管理**:
    - もし何らかの理由でフォールバック値が必要な場合でも、それをハードコードするのではなく、設定ファイル（例: `settings.py` または専用のコンフィグレーション）で管理し、システムの起動時に明示的に設定できるようにします。これにより、値の変更や影響範囲の特定が容易になります。

**期待される効果:**

- **バックテストの信頼性向上**: 実際の市場状況をより正確に反映したバックテスト結果が得られます。
- **問題の早期発見**: データ不足の問題が隠蔽されず、より早期に発見・対処できるようになります。
- **コードの透明性**: データハンドリングのロジックがより明確になり、コードの可読性が向上します。
- **堅牢なシステム**: データの一時的な欠落に対する回復力が高まります。
