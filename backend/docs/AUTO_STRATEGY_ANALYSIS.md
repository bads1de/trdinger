...
### 4.2 インジケータパラメータの依存関係検証不足 (Parameter Dependency Issue)

**現状:**

- `GeneValidator` では個々のパラメータの型や範囲チェックは行っていますが、パラメータ間の論理的整合性（例: MACD における `fast_period < slow_period`、BB における `upper_band > lower_band`）をチェックするロジックが欠如しています。

**影響:**

- **学習効率の低下**: 無効なパラメータ設定（例: Fast=50, Slow=10）を持つ個体が生成され、バックテスト実行エラー（または意図しない挙動）を引き起こします。これらは単に「フィットネス 0」として処理されるため、GA は「なぜ悪いのか」を学習できず、無駄な探索を繰り返す可能性があります。
