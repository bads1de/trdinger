...
### 2. `backend/app/utils/data_processing.py`

**📏 規模:** 1358 行
**🔍 問題点:**

- **巨大な DataProcessor クラス**（800+行）
- **過多な責任**：
  - データ前処理
  - 外れ値除去
  - カテゴリカル変数エンコーディング
  ...
  - 型最適化
- **複雑なメソッド**：
  - `interpolate_columns`: 100+行
  - `prepare_training_data`: 70+行
- **scikit-learn Pipeline との混合**
...
data_processing/
├── transformers/
│   ├── outlier_remover.py           # 外れ値除去専用Transformer
│   ├── categorical_encoder.py       # カテゴリエンコーディング専用
│   ├── data_imputer.py             # 補間処理専用
│   └── dtype_optimizer.py          # 型最適化専用
├── pipelines/
│   ├── preprocessing_pipeline.py   # 前処理Pipeline
│   ├── ml_pipeline.py              # ML用Pipeline
│   └── comprehensive_pipeline.py   # 包括的Pipeline
├── validators/
│   └── data_validator.py           # データ検証専用
└── data_processor.py               # 統合インターフェースのみ
```

**🎯 期待効果:**

- コード行数: 1358 行 → 400 行（約 70%削減）
- Pipeline 活用: 宣言的処理による保守性向上
- 再利用性: Transformer の個別利用可能
...
```python
# 変更前: 巨大メソッド
def _calculate_with_pandas_ta(self, df, config, params):  # 216行

# 変更後: 4つの簡素メソッド
def _get_config(self, indicator_type):                   # 設定取得
def _basic_validation(self, df, config):                # 検証
def _normalize_params(self, params, config):            # 正規化
def _call_pandas_ta(self, df, config, params):          # 実行
```
