...
class TestMLSystemIssues:
    ...
    def sample_funding_rate_data(self, sample_ohlcv_data):
        ...
        return pd.DataFrame({
            ...
            'funding_rate': np.random.normal(0.0001, 0.0005, len(sample_ohlcv_data))
        }).set_index('timestamp')
    ...
    def sample_open_interest_data(self, sample_ohlcv_data):
        ...
        return pd.DataFrame({
            'timestamp': sample_ohlcv_data.index,
            'open_interest': np.random.uniform(1000000, 5000000, len(sample_ohlcv_data))
        }).set_index('timestamp')

    def test_feature_scaling_disabled_issue(self, sample_ohlcv_data, sample_funding_rate_data, sample_open_interest_data):
        """
        ...
        for col, stats in feature_stats.items():
            logger.info(f"  {col}: 平均={stats['mean']:.4f}, 標準偏差={stats['std']:.4f}, 範囲={stats['range']:.4f}")
        ...
        if len(ranges) > 1:
            ...
            assert scale_ratio > 100, f"特徴量のスケール不整合が検出されました（比率: {scale_ratio:.2f}）"
            logger.warning(f"⚠️ 特徴量スケーリングが無効のため、スケール不整合が発生しています（比率: {scale_ratio:.2f}）")
    ...
    def test_zscore_outlier_detection_issue(self, sample_ohlcv_data):
        ...
        test_data.iloc[crash_index:crash_index+5, test_data.columns.get_loc('Close')] *= 0.8  # 20%下落
        ...
        if outlier_count > 0:
            logger.warning(f"⚠️ Z-score外れ値検出により{outlier_count}個の重要な市場シグナルが除去される可能性があります")
    ...
    def test_fixed_threshold_label_generation_issue(self, sample_ohlcv_data):
        ...
        logger.info(f"現在の固定閾値: 上昇={fixed_threshold_up}, 下落={fixed_threshold_down}")
        ...
        # 固定閾値の分布分析
        fixed_dist, fixed_imbalance = analyze_label_distribution(labels_fixed, "固定閾値")
        
        # 動的閾値の分布分析
        dynamic_dist, dynamic_imbalance = analyze_label_distribution(labels_dynamic, "動的閾値")
        ...
        if fixed_imbalance > 3.0:  # 3倍以上の不均衡
            logger.warning(f"⚠️ 固定閾値により深刻なクラス不均衡が発生しています（比率: {fixed_imbalance:.2f}）")
    ...
    def test_time_series_cv_missing_issue(self):
        ...
        logger.warning("⚠️ 現在のシステムでは時系列データに適したクロスバリデーションが実装されていません")
    ...
    def test_overall_system_impact(self, sample_ohlcv_data, sample_funding_rate_data, sample_open_interest_data):
        """
        全体的なシステムへの影響を検証
        """
        ...
        label_stats = self.test_fixed_threshold_label_generation_issue(sample_ohlcv_data)
...
if __name__ == "__main__":
    ...
    # 全体的な影響を検証
    results = test_instance.test_overall_system_impact(sample_data, funding_data, oi_data)
