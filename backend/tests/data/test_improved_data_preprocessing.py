...
class TestImprovedDataPreprocessing:
    ...
    def generate_financial_data_with_events(self):
        ...
        data = {
            ...
            "volatility": np.abs(base_returns)
            * np.random.uniform(0.8, 1.2, len(dates)),
    ...
    def test_iqr_vs_zscore_outlier_detection(self):
        ...
        def calculate_event_preservation(original_data, processed_data, events):
            ...
            for event_type, indices in events.items():
                if isinstance(indices, list):
                    ...
                    preservation_rate = (
                        preserved_count / total_count if total_count > 0 else 0
                    )
                    ...
                    idx = indices
                    if idx < len(processed_data):
                        preserved = not pd.isna(
                            processed_data.iloc[idx]["price_change"]
                        )
                        preservation_stats[event_type] = {
        ...
        # 各方法での重要イベント保持率を計算
        zscore_preservation = calculate_event_preservation(
            financial_data, data_zscore, important_events
        )
        iqr_preservation = calculate_event_preservation(
            financial_data, data_iqr, important_events
        )
        ...
        for event_type, stats in zscore_preservation.items():
            logger.info(
                f"  {event_type}: {stats['preserved']}/{stats['total']} ({stats['rate']*100:.1f}%)"
            )
        ...
        # 全体的な保持率
        zscore_overall_rate = (
            total_zscore_preserved / total_zscore_events
            if total_zscore_events > 0
            else 0
        )
        iqr_overall_rate = (
            total_iqr_preserved / total_iqr_events if total_iqr_events > 0 else 0
        )
        ...
        if iqr_overall_rate > zscore_overall_rate:
            logger.info(
                "✅ IQRベースの外れ値検出により、重要な金融イベントがより多く保持されました"
            )
        else:
    ...
    def test_financial_data_characteristics(self):
        ...
        for method in methods:
            ...
            for col in financial_features.columns:
                ...
                preservation_rate = (
                    preserved_count / original_count if original_count > 0 else 0
                )
            ...
            logger.info(
                f"  全体平均: {results[method]['overall_preservation']*100:.1f}%"
            )
    ...
    def test_data_quality_monitoring(self):
        ...
        problematic_data = pd.DataFrame(
            {
                ...
                "missing_heavy": np.concatenate(
                    [np.random.normal(0, 1, 50), [np.nan] * 50]
                ),
                "outlier_heavy": np.concatenate(
                    [np.random.normal(0, 1, 90), [100, -100] * 5]
                ),
                "zero_variance": [1.0] * 100,
                "infinite_values": np.concatenate(
                    [
                        np.random.normal(0, 1, 95),
                        [np.inf, -np.inf, np.nan, np.inf, -np.inf],
                    ]
                ),
            }
        )
        ...
        def analyze_data_quality(df, label):
            ...
            for col in df.columns:
                ...
                logger.info(f"  {col}:")
                logger.info(
                    f"    欠損値: {missing_count}/{total_count} ({missing_count/total_count*100:.1f}%)"
                )
        ...
        improvement_rate = (
            total_improvements / max_possible_improvements
    ...
    def test_overall_preprocessing_improvement(self):
        ...
        zscore_financial_preservation = financial_results["zscore"][
...
if __name__ == "__main__":
    ...
    print(
        f"重要イベント保持率（IQR）: {results['outlier_results']['iqr_overall_rate']*100:.1f}%"
    )
    print(
        f"重要イベント保持率（Z-score）: {results['outlier_results']['zscore_overall_rate']*100:.1f}%"
    )
    print(
        f"データ品質改善率: {results['quality_results']['improvement_rate']*100:.1f}%"
    )
