...
class TestMLPipelinesComprehensive:
    ...
    def test_invalid_classification_selection_method_error(self):
        """無効な分類特徴量選択方法のテスト"""
        with pytest.raises(ValueError, match="Unsupported classification selection method"):
            create_classification_pipeline(
                feature_selection=True,
                n_features=2,
                selection_method='invalid_method'
    ...
    def test_pipeline_info_completeness(self, sample_data):
        ...
        required_fields = [
            'pipeline_type', 'n_steps', 'step_names',
            'has_preprocessing', 'has_feature_selection', 'has_scaling'
        ]
