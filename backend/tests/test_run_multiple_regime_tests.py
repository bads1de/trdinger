...
class TestRunMultipleRegimeTests(unittest.TestCase):
    ...
    def test_run_multiple_tests_structure(self, mock_comparison):
        ...
        mock_comparison.return_value = {
            'regime_results': {
                'trend': {'performance_metrics': {'sharpe_ratio': 1.5, 'max_drawdown': -0.1}},
                'range': {'performance_metrics': {'sharpe_ratio': 0.8, 'max_drawdown': -0.05}}
            },
            'summary': {}
        }
    ...
    def test_statistics_calculation(self, mock_comparison):
        ...
        mock_comparison.side_effect = [
            ...
            {
                'regime_results': {
                    'trend': {'performance_metrics': {'sharpe_ratio': 1.0, 'max_drawdown': -0.1}}
                },
                'summary': {}
            },
            # enabled run2
            {
                'regime_results': {
                    'trend': {'performance_metrics': {'sharpe_ratio': 1.2, 'max_drawdown': -0.2}}
                },
                'summary': {}
            },
            # disabled run1
            {
                'regime_results': {
                    'trend': {'performance_metrics': {'sharpe_ratio': 0.5, 'max_drawdown': -0.3}}
                },
                'summary': {}
            },
            # disabled run2
            {
                'regime_results': {
                    'trend': {'performance_metrics': {'sharpe_ratio': 0.7, 'max_drawdown': -0.4}}
                },
                'summary': {}
            }
        ]
    ...
    def test_csv_output(self):
        ...
        with patch('scripts.run_multiple_regime_tests.regime_based_backtest_comparison') as mock_comp:
