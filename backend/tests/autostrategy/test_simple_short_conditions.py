...
def test_simple_short_conditions():
    ...
    try:
        ...
        with SessionLocal() as db:
            ...
            # バックテスト設定（短期間でテスト）
            config = {
                "strategy_name": "Test_Simple_Short_Strategy",
                "symbol": "BTC/USDT:USDT",
                "timeframe": "1h",
                "start_date": datetime.now() - timedelta(days=7),  # 1週間
            ...
            if len(trade_history) > 0:
                ...
                if len(long_trades) > 0:
                    print(f"   ロング取引例:")
                    for i, trade in enumerate(long_trades[:3]):  # 最初の3つを表示
                        print(
                            f"   ロング取引 {i+1}: サイズ={trade.get('size', 0)}, エントリー価格={trade.get('entry_price', 0)}"
                        )
