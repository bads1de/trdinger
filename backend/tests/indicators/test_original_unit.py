...
class TestOriginalUnitExtended:
    ...
    def test_original_all_methods_and_aliases(self, sample_df):
        ...
        assert isinstance(OriginalIndicators.calculate_elder_ray(sample_df), pd.DataFrame)
        
        # 2. Adaptive Entropy & alias
        assert len(OriginalIndicators.adaptive_entropy(c)) == 3
        assert isinstance(OriginalIndicators.calculate_adaptive_entropy(sample_df), pd.DataFrame)
        
        # 3. Quantum Flow & alias
        assert len(OriginalIndicators.quantum_flow(c, h, l, v)) == 2
        assert isinstance(OriginalIndicators.calculate_quantum_flow(sample_df), pd.DataFrame)
        
        # 4. Harmonic Resonance & alias
        assert len(OriginalIndicators.harmonic_resonance(c, h, l)) == 2
        assert isinstance(OriginalIndicators.calculate_harmonic_resonance(sample_df), pd.DataFrame)
        
        # 5. Chaos Fractal Dimension & alias
        assert len(OriginalIndicators.chaos_fractal_dimension(c, h, l, v)) == 2
        assert isinstance(OriginalIndicators.calculate_chaos_fractal_dimension(sample_df), pd.DataFrame)
        
        # 6. Prime Oscillator & alias
        assert len(OriginalIndicators.prime_oscillator(c)) == 2
        assert isinstance(OriginalIndicators.calculate_prime_oscillator(sample_df), pd.DataFrame)
        
        # 7. Fibonacci Cycle & alias
        assert len(OriginalIndicators.fibonacci_cycle(c)) == 2
        assert isinstance(OriginalIndicators.calculate_fibonacci_cycle(sample_df), pd.DataFrame)
        
        # 8. McGinley Dynamic & alias
        assert isinstance(OriginalIndicators.mcginley_dynamic(c), pd.Series)
        # 内部で単一カラムのDataFrameとして返ってくる
        assert isinstance(OriginalIndicators.calculate_mcginley_dynamic(sample_df), (pd.Series, pd.DataFrame))
        
        # 9. Chande Kroll Stop & alias
        assert len(OriginalIndicators.chande_kroll_stop(h, l, c)) == 2
        assert isinstance(OriginalIndicators.calculate_chande_kroll_stop(sample_df), pd.DataFrame)
        
        # 10. Trend Intensity Index & alias
        assert isinstance(OriginalIndicators.trend_intensity_index(c, h, l), pd.Series)
        assert isinstance(OriginalIndicators.calculate_trend_intensity_index(sample_df), (pd.Series, pd.DataFrame))
        
        # 11. Connors RSI & alias
        assert isinstance(OriginalIndicators.connors_rsi(c), pd.Series)
        assert isinstance(OriginalIndicators.calculate_connors_rsi(sample_df), (pd.Series, pd.DataFrame))
    ...
    def test_frama_edge_cases(self, sample_df):
        c = sample_df["close"]
        # パラメータエラー
        with pytest.raises(ValueError): OriginalIndicators.frama(c, length=3)
        with pytest.raises(ValueError): OriginalIndicators.frama(c, length=5) # 奇数
        with pytest.raises(ValueError): OriginalIndicators.frama(c, slow=0)
    ...
    def test_ichimoku_fallback_direct(self, sample_df):
        ...
        # pandas-taが完全に失敗して例外を投げる状況をシミュレート
        with patch("pandas_ta.ichimoku", side_effect=RuntimeError("Internal Ta-Lib Error")):
