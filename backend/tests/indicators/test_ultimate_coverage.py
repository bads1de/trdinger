...
class TestUltimateCoverage:
    ...
    def test_momentum_complex_params(self, sample_df):
        ...
        res = MomentumIndicators.trix(
            c, length=10, signal=5, scalar=1.0, drift=2, offset=1
        )
        assert len(res) == 3
    ...
    def test_trend_volatility_ma_modes(self, sample_df):
        ...
        # 1. BIAS の全 MA タイプを再確認 (MomentumIndicatorsに移動済み)
        ...
        # 3. RVI の詳細パラメータ
        assert isinstance(
            VolatilityIndicators.rvi(c, h, l, refined=True, thirds=True), pd.Series
        )
