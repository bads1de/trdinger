...
class TestTrendCoverage:
    ...
    def test_all_trend_indicators_registered(self):
        all_indicators = indicator_registry.get_all_indicators()
        trend_indicators = [name for name, config in all_indicators.items() if config.category == "trend"]
        ...
        if failures:
            pytest.fail(f"Failed to calculate some trend indicators:\n" + "\n".join(failures))