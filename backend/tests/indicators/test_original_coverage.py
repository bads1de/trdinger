...
class TestOriginalCoverage:
    ...
    def setup(self):
        ...
        self.data = pd.DataFrame({
            ...
            "open_interest": np.linspace(10000, 20000, rows) + np.random.normal(0, 1000, rows),
            "funding_rate": np.random.normal(0.0001, 0.0001, rows),
            "market_cap": np.linspace(1e9, 1.1e9, rows)
        })

    def test_all_original_indicators_registered(self):
        all_indicators = indicator_registry.get_all_indicators()
        original_indicators = [name for name, config in all_indicators.items() if config.category in ["original", "advanced"]]
        
        unique_indicators = {}
        for name in original_indicators:
        ...
        if failures:
            pytest.fail(f"Failed to calculate some original indicators:\n" + "\n".join(failures))
