...
class TestVolumeCoverage:
    ...
    def test_all_volume_indicators_registered(self):
        all_indicators = indicator_registry.get_all_indicators()
        volume_indicators = [name for name, config in all_indicators.items() if config.category == "volume"]
        ...
        if failures:
            pytest.fail(f"Failed to calculate some volume indicators:\n" + "\n".join(failures))
