...
def test_corrected_strategy_gene():
    ...
    try:
        ...
        entry_conditions = [
            ...
            Condition(left_operand="RSI_14", operator="<", right_operand=35),  # 売られすぎからの反発
            Condition(left_operand="FundingRate", operator=">", right_operand=0.0005),  # ロング過熱時のショート
            Condition(left_operand="OpenInterest", operator=">", right_operand=10000000),  # 大きなOIでトレンド確認
        ]
        
        exit_conditions = [
            Condition(left_operand="close", operator="<", right_operand="SMA_20"),
            Condition(left_operand="RSI_14", operator=">", right_operand=70),  # 買われすぎで利確
            Condition(left_operand="FundingRate", operator="<", right_operand=-0.0001),  # センチメント変化
        ]
...
def test_corrected_random_generator():
    ...
    try:
                ...
                for cond in all_conditions:
                    if cond.left_operand in ["OpenInterest", "FundingRate"] or \
                       (isinstance(cond.right_operand, str) and 
                        ...
                        oi_fr_conds.append(f"{cond.left_operand} {cond.operator} {cond.right_operand}")
        ...
        # 4. 統計結果表示
        print(f"\n4. 統計結果:")
        print(f"  📊 有効戦略率: {valid_strategies}/{len(population)} ({valid_strategies/len(population)*100:.1f}%)")
        print(f"  📊 OI/FR判断使用率: {oi_fr_usage}/{total_conditions} ({oi_fr_usage/total_conditions*100:.1f}%)")
        ...
        invalid_indicators = [t for t in indicator_types.keys() 
                            if t in ["OpenInterest", "FundingRate"] or 
                               t.startswith(("OI_", "FR_"))]
...
def test_fitness_focus():
    ...
    try:
        ...
        technical_only = all(
            indicator.type not in ["OpenInterest", "FundingRate"] and
            not indicator.type.startswith(("OI_", "FR_"))
