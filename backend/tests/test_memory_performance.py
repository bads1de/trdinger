...
class TestMemoryPerformance:
    ...
    def test_memory_usage_multiple_indicators(self, medium_dataset):
        ...
        assert memory_increase < 200, f"Memory increase too large: {memory_increase:.2f} MB"
    ...
    def test_performance_single_indicator(self, medium_dataset):
        ...
        # 平均処理時間が1秒以下であることを確認
        assert avg_time < 1.0, f"Performance too slow: {avg_time:.3f} seconds per calculation"
    ...
    def test_performance_multiple_indicators(self, medium_dataset):
        ...
        assert avg_time_per_indicator < 0.5, f"Performance too slow: {avg_time_per_indicator:.3f} seconds per indicator"
    ...
    def test_series_vs_array_performance(self, medium_dataset):
        ...
        assert ratio < 5.0, f"Performance difference too large: Series={series_time:.3f}s, Array={array_time:.3f}s"
    ...
    def test_concurrent_performance(self, medium_dataset):
        ...
        assert avg_time < 2.0, f"Concurrent performance too slow: {avg_time:.3f} seconds average"
    ...
    def test_auto_strategy_performance(self, medium_dataset):
        ...
        assert avg_time_per_calculation < 0.3, f"Auto strategy performance too slow: {avg_time_per_calculation:.3f} seconds"
    ...
    def test_garbage_collection_efficiency(self, medium_dataset):
        ...
        assert memory_increase < 100, f"Memory not properly released: {memory_increase:.2f} MB increase"
    ...
    def test_data_size_scalability(self):
        ...
        assert times[-1] < 5.0, f"Scalability issue: {times[-1]:.3f} seconds for {sizes[-1]} records"
        
        # 処理時間の増加率が合理的であることを確認
        ...
        for time_ratio, size_ratio in zip(time_ratios, size_ratios):
            ...
            assert time_ratio <= size_ratio * 2, f"Non-linear scaling detected: time ratio {time_ratio:.2f}, size ratio {size_ratio:.2f}"
