...
class TestMigrationEdgeCases:
    ...
    def test_missing_required_columns(self):
        ...
        result = self.service.calculate_indicator(incomplete_data, 'SMA', {'period': 20})
        assert isinstance(result, np.ndarray)
    ...
    def test_data_type_consistency(self):
        ...
        # 価格の整合性を保証
        int_data['High'] = np.maximum(int_data['High'], np.maximum(int_data['Open'], int_data['Close']))
        int_data['Low'] = np.minimum(int_data['Low'], np.minimum(int_data['Open'], int_data['Close']))
    ...
    def test_memory_stress(self):
        ...
        assert memory_increase < 1000, f"メモリ使用量が過大です: {memory_increase:.2f}MB"
