...
from app.core.services.indicators.trend_indicators import (
    SMAIndicator, EMAIndicator, T3Indicator, TEMAIndicator
)
from app.core.services.indicators.momentum_indicators import (
    RSIIndicator, MomentumIndicator, CMOIndicator, PPOIndicator
)
from app.core.services.indicators.volatility_indicators import (
    BollingerBandsIndicator, ATRIndicator, STDDEVIndicator
)
...
class TestIndicatorCalculationRefactoring:
    ...
    def test_sma_indicator_uses_talib_adapter(self, sample_ohlcv_data):
        ...
        with patch.object(TALibAdapter, 'sma') as mock_sma:
            # TALibAdapterのsmaメソッドが期待される結果を返すようにモック
            expected_result = pd.Series([100.0] * len(sample_ohlcv_data), 
                                      index=sample_ohlcv_data.index, name='SMA_20')
            mock_sma.return_value = expected_result
            
            # SMAIndicatorのcalculateメソッドを呼び出し
            result = indicator.calculate(sample_ohlcv_data, period)
            
            # TALibAdapter.smaが正しい引数で呼び出されたことを検証
            mock_sma.assert_called_once()
            call_args = mock_sma.call_args
            
            # 第1引数（データ）がclose価格であることを確認
    ...
    def test_ema_indicator_uses_talib_adapter(self, sample_ohlcv_data):
        ...
        with patch.object(TALibAdapter, 'ema') as mock_ema:
            expected_result = pd.Series([100.0] * len(sample_ohlcv_data), 
                                      index=sample_ohlcv_data.index, name='EMA_20')
    ...
    def test_rsi_indicator_uses_talib_adapter(self, sample_ohlcv_data):
        ...
        with patch.object(TALibAdapter, 'rsi') as mock_rsi:
            expected_result = pd.Series([50.0] * len(sample_ohlcv_data), 
                                      index=sample_ohlcv_data.index, name='RSI_14')
    ...
    def test_atr_indicator_uses_talib_adapter(self, sample_ohlcv_data):
        ...
        with patch.object(TALibAdapter, 'atr') as mock_atr:
            expected_result = pd.Series([1.0] * len(sample_ohlcv_data), 
                                      index=sample_ohlcv_data.index, name='ATR_14')
            ...
            # ATRは3つの価格データ（high, low, close）を使用
            pd.testing.assert_series_equal(call_args[0][0], sample_ohlcv_data['high'])
            pd.testing.assert_series_equal(call_args[0][1], sample_ohlcv_data['low'])
            pd.testing.assert_series_equal(call_args[0][2], sample_ohlcv_data['close'])
    ...
    def test_bollinger_bands_indicator_uses_talib_adapter(self, sample_ohlcv_data):
        ...
        with patch.object(TALibAdapter, 'bollinger_bands') as mock_bb:
            expected_result = {
                'upper': pd.Series([105.0] * len(sample_ohlcv_data), index=sample_ohlcv_data.index),
                'middle': pd.Series([100.0] * len(sample_ohlcv_data), index=sample_ohlcv_data.index),
                'lower': pd.Series([95.0] * len(sample_ohlcv_data), index=sample_ohlcv_data.index)
            }
    ...
    def test_no_direct_talib_imports_in_indicators(self):
        ...
        import inspect
        from app.core.services.indicators import trend_indicators, momentum_indicators, volatility_indicators
        
        # 各モジュールのソースコードを取得
        modules = [trend_indicators, momentum_indicators, volatility_indicators]
        
        for module in modules:
            source = inspect.getsource(module)
            # 直接的なTA-Libインポートがないことを確認
            assert 'import talib' not in source, f"{module.__name__} contains direct talib import"
            assert 'from talib' not in source, f"{module.__name__} contains direct talib import"
    ...
    def test_adapters_not_used_in_indicators(self):
        ...
        for module in modules:
            source = inspect.getsource(module)
            # 個別アダプターの使用がないことを確認
            assert 'TrendAdapter' not in source, f"{module.__name__} still uses TrendAdapter"
            assert 'MomentumAdapter' not in source, f"{module.__name__} still uses MomentumAdapter"
            assert 'VolatilityAdapter' not in source, f"{module.__name__} still uses VolatilityAdapter"
    ...
    def test_talib_adapter_usage_in_indicators(self):
        ...
        for module in modules:
            ...
            assert 'TALibAdapter' in source, f"{module.__name__} does not use TALibAdapter"
