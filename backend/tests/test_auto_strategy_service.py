...
def auto_strategy_service(
    ...
    with patch("app.services.auto_strategy.services.auto_strategy_service.SessionLocal", mock_db_session_factory), \
         patch("app.services.auto_strategy.services.auto_strategy_service.BacktestService", return_value=mock_backtest_service), \
         patch("app.services.auto_strategy.services.auto_strategy_service.ExperimentPersistenceService", return_value=mock_persistence_service), \
         patch("app.services.auto_strategy.services.auto_strategy_service.ExperimentManager", return_value=mock_experiment_manager):
...
def test_start_strategy_generation_success(auto_strategy_service, mock_persistence_service, mock_experiment_manager):
    """正常系: 戦略生成が正常に開始されること"""
    # 準備 - GA設定を有効な値に修正
    ...
    result_exp_id = auto_strategy_service.start_strategy_generation(
        experiment_id, experiment_name, ga_config_dict, backtest_config_dict, background_tasks
    )
...
def test_start_strategy_generation_invalid_ga_config(auto_strategy_service):
    ...
    # 準備
    with patch("app.services.auto_strategy.config.GAConfig.validate", return_value=(False, ["error"])):
...
def test_stop_experiment_manager_not_initialized(auto_strategy_service):
    ...
    # 検証
    assert result == {"success": False, "message": "実験管理マネージャーが初期化されていません"}
...
def test_create_experiment_called_with_correct_args(auto_strategy_service, mock_persistence_service):
    ...
    auto_strategy_service._create_experiment(experiment_id, experiment_name, ga_config, backtest_config)
...
def test_initialize_ga_engine_runtime_error(auto_strategy_service):
    ...
    auto_strategy_service.experiment_manager = None
    with pytest.raises(RuntimeError, match="実験管理マネージャーが初期化されていません。"):
        auto_strategy_service._initialize_ga_engine(GAConfig.from_dict({})) # Assuming GAConfig has a from_dict method
