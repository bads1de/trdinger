...
class TestRegimeDetector:
    ...
    def test_detect_regime_basic(self, detector):
        ...
        assert all(
            regime in [0, 1, 2] for regime in regimes
        )  # 0=トレンド、1=レンジ、2=高ボラ
    ...
    def test_regime_transition_validation(self, detector):
        ...
        trend_data = np.array(
            [
                [i * 0.01, i * 0.01 + 0.005, i * 0.01 - 0.005, i * 0.01 + 0.002]
        ...
        range_data = np.array(
            [
                [0.5 + np.random.randn() * 0.001 for _ in range(4)]
    ...
    def test_invalid_data_handling(self, detector):
        ...
        # 無限大を含むデータ
        invalid_data_inf = pd.DataFrame(
            [[1.0, 2.0, np.inf, 4.0, 100.0]],
            columns=["open", "high", "low", "close", "volume"],
        )
        with pytest.raises(ValueError):
    ...
    def test_model_fitting_error_handling(self, detector):
        ...
        data = pd.DataFrame(
            np.random.randn(10, 5), columns=["open", "high", "low", "close", "volume"]
        )
        with patch.object(
            detector.model, "fit", side_effect=Exception("Fitting failed")
        ):
    ...
    def test_regime_label_mapping(self, detector):
        ...
        # モックを使用して特定の状態を返す
        with patch.object(
            detector.model, "predict", return_value=np.array([0, 1, 2, 0, 1])
        ):
            data = pd.DataFrame(
                np.random.randn(5, 5),
                columns=["open", "high", "low", "close", "volume"],
