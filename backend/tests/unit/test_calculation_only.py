...
# from app.core.services.technical_indicator_service import TechnicalIndicatorService  # 削除済み
...
class CalculationTester:
    ...
    def create_test_data(
        self, length: int = 100, pattern: str = "random"
    ) -> pd.DataFrame:
        """テスト用のOHLCVデータを生成"""
        dates = pd.date_range(
        ...
        if pattern == "random":
            ...
            base_price = 50000
            prices = [
                base_price + i * 100 + np.random.normal(0, 50) for i in range(length)
            ]
            ...
            for indicator_type, config in self.service.supported_indicators.items():
                try:
                    ...
                    if isinstance(result, pd.DataFrame):
                        ...
                        print(
                            f"  ✅ {indicator_type:6s}: DataFrame ({valid_rows}有効行)"
                        )
