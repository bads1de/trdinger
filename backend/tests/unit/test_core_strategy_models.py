    ...
    try:
        ...
        for i, condition in enumerate(valid_conditions):
            if condition.validate():
                print(
                    f"  ✅ 条件{i+1}: {condition.left_operand} {condition.operator} {condition.right_operand} - 有効"
                )
            else:
                print(f"  ❌ 条件{i+1}: 無効")
                return False
    ...
    try:
        ...
        for indicator in gene.indicators:
            if indicator.type in [
                "OpenInterest",
                "FundingRate",
            ] or indicator.type.startswith(("OI_", "FR_")):
        ...
        for i, individual in enumerate(population):
            ...
            print(
                f"    個体{i+1}: {'✅' if is_valid else '❌'} {'(OI/FR使用)' if has_oi_fr else ''}"
            )

        print(
            f"  📊 有効個体率: {valid_count}/{len(population)} ({valid_count/len(population)*100:.1f}%)"
        )
        print(
            f"  📊 OI/FR活用率: {oi_fr_usage_count}/{len(population)} ({oi_fr_usage_count/len(population)*100:.1f}%)"
        )
...
if __name__ == "__main__":
    ...
    success4 = test_corrected_models()
    success5 = test_ga_objectives_summary()
