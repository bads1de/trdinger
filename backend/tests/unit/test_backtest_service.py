...
class TestBacktestService:
    ...
    def test_run_backtest_success(
        self, backtest_service, mock_data_service, sample_config, sample_ohlcv_dataframe
    ):
        """正常なバックテスト実行テスト"""
        # モックの設定
    ...
    def test_run_backtest_with_custom_strategy_parameters(
        self, backtest_service, mock_data_service, sample_ohlcv_dataframe
    ):
    ...
    def test_run_backtest_no_data_error(
        self, backtest_service, mock_data_service, sample_config
    ):
        """データが見つからない場合のエラーテスト"""
        # モックの設定（例外を発生させる）
        mock_data_service.get_ohlcv_for_backtest.side_effect = ValueError(
            "No data found"
        )
    ...
    def test_create_strategy_class_invalid_type(self, backtest_service):
        """無効な戦略タイプのテスト"""
        strategy_config = {"strategy_type": "INVALID_STRATEGY", "parameters": {}}
    ...
    def test_convert_backtest_results(self, backtest_service):
        ...
        equity_curve = pd.DataFrame(
            {"Equity": [100000, 101000, 102000], "DrawdownPct": [0.0, -0.01, -0.005]},
    ...
    def test_validate_backtest_config_invalid_dates(self, backtest_service):
        ...
        # エラーが発生することを確認
        with pytest.raises(
            ValueError, match="開始日は終了日よりも前である必要があります"
        ):
    ...
    def test_run_backtest_with_backtesting_py(
        self,
        mock_backtest_class,
        backtest_service,
        mock_data_service,
