...
def cleanup_test_profiles():
    ...
    try:
        ...
        for profile in all_profiles:
            if any(keyword in profile.profile_name.lower() for keyword in 
                   ['test', 'lightgbm_optimized', 'scenario']):
...
async def main():
    ...
    try:
        ...
        for result in all_results:
            print(f"  ID: {result.id}, Name: {result.profile_name}, Model: {result.model_type}, Default: {result.is_default}")
    finally:
        db.close()
