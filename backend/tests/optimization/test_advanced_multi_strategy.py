...
def generate_market_scenario_data(
    scenario: str,
    start_date: datetime,
    end_date: datetime,
    ...
    if scenario == "trending_up":
        ...
        daily_returns = np.random.normal(0.002, 0.02, len(date_range)) + trend / len(
            date_range
        )
        ...
        daily_returns = np.random.normal(-0.001, 0.02, len(date_range)) + trend / len(
            date_range
        )
...
def insert_scenario_data_to_db(
...
def test_strategy_robustness():
    ...
    strategies = {
        "SMA_CROSS": {"n1": [10, 20], "n2": [30, 50]},
        "RSI": {"period": [14, 21], "oversold": [25, 35], "overbought": [65, 75]},
        "MACD": {
            "fast_period": [10, 15],
            "slow_period": [20, 30],
            "signal_period": [7, 11],
...
def analyze_robustness_results(results):
    ...
    for strategy_type, strategy_results in results.items():
        ...
        if valid_results:
            ...
            avg_drawdown = np.mean(
                [r["metrics"]["max_drawdown"] for r in valid_results]
            )
    ...
    for strategy_type, scores in strategy_scores.items():
        print(
            f"{strategy_type}\t\t{scores['avg_sharpe']:.3f}\t\t{scores['avg_return']:.2f}%\t\t{scores['avg_drawdown']:.2f}%\t\t{scores['success_rate']:.0f}%\t\t{scores['robustness_score']:.3f}"
        )
...
def test_market_condition_analysis():
    ...
    for scenario in scenarios:
        ...
        # SMA_CROSS
        result = test_strategy_in_scenario(
            "SMA_CROSS", scenario, {"n1": [15, 25], "n2": [40, 60]}
        )
        if result:
        ...
        # RSI
        result = test_strategy_in_scenario(
            "RSI", scenario, {"period": [14], "oversold": [30], "overbought": [70]}
        )
        if result:
        ...
        if scenario_results:
            ...
            print(
                f"  最適戦略: {best_strategy[0]} (シャープレシオ: {best_strategy[1]:.3f})"
            )
        else:
            print("  有効な結果がありません")
