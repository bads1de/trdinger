...
class TestDataLeakPrevention:
    ...
    def mock_ml_config(self):
        ...
        with patch(
            "app.config.unified_config.unified_config.ml.training"
        ) as mock_ml_training_config:
            ...
            mock_ml_training_config.use_purged_kfold = (
                True  # PurgedKFoldが有効になるように
            )
    ...
    def test_purged_kfold_removes_overlapping_samples(self):
        ...
        for fold, (train_idx, test_idx) in enumerate(pkf.split(X)):
            ...
            if len(train_indices) > 0 and len(test_indices) > 0:
                ...
                assert max_train_t1 < min_test_start, (
                    f"フォールド{fold}: 学習データの終了時刻(max_train_t1={max_train_t1})が"
                    f"テストデータの開始時刻(min_test_start={min_test_start})以降にあります。"
                    "Purged K-Foldが正しく機能していません。"
                )
