...
class ConcreteFeatureCalculator(BaseFeatureCalculator):
    ...
    def calculate_features(self, df: pd.DataFrame, config: dict) -> pd.DataFrame:
        """具象実装"""
...
class TestBaseFeatureCalculator:
    ...
    def sample_data(self):
        """サンプルデータ"""
        dates = pd.date_range(start=datetime(2023, 1, 1), periods=100, freq="1h")
    ...
    def test_batch_calculate_ratio_division_by_zero(self):
        ...
        assert (
            pd.isna(result["test_ratio"].iloc[1]) or result["test_ratio"].iloc[1] == 0
        )
