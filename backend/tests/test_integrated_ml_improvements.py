...
class TestIntegratedMLImprovements:
    ...
    def test_integrated_system_before_vs_after(self):
        ...
        def simulate_old_system():
            ...
            old_scale_ratio = max(ranges) / min([r for r in ranges if r > 0]) if len(ranges) > 1 else 1
            ...
            ratios = [old_label_dist['down'], old_label_dist['range'], old_label_dist['up']]
            max_ratio = max(ratios)
            min_ratio = min([r for r in ratios if r > 0])
            old_imbalance_ratio = max_ratio / min_ratio if min_ratio > 0 else float('inf')
        ...
        def test_new_system():
            ...
            new_scale_ratio = max(ranges) / min([r for r in ranges if r > 0]) if len(ranges) > 1 else 1
            ...
            ratios = [new_label_dist['down'], new_label_dist['range'], new_label_dist['up']]
            max_ratio = max(ratios)
            min_ratio = min([r for r in ratios if r > 0])
            new_imbalance_ratio = max_ratio / min_ratio if min_ratio > 0 else float('inf')
        ...
        imbalance_improvement = old_results['imbalance_ratio'] / new_results['imbalance_ratio']
        ...
        assert scale_improvement > 10, f"スケール改善が不十分: {scale_improvement:.2f}倍"
        assert imbalance_improvement > 1.5, f"クラス不均衡改善が不十分: {imbalance_improvement:.2f}倍"
        assert new_results['imbalance_ratio'] < 4.0, f"クラス不均衡が深刻: {new_results['imbalance_ratio']:.2f}"
    ...
    def test_configuration_integration(self):
        ...
        # 全ての設定が正しいかを確認
        all_configs_correct = all(status['matches'] for status in config_status.values())
        ...
        return {
            ...
            'all_correct': all_configs_correct
        }
