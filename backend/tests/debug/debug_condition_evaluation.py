...
def debug_condition_evaluation():
    """条件評価のデバッグ"""
    print("=" * 60)
    print("Condition Evaluation Debug")
    print("=" * 60)
    ...
    try:
        ...
        with SessionLocal() as db:
            ...
            # より緩い条件を作成（取引が発生するように）
            entry_conditions = [
                Condition(
                    left_operand="RSI",
                    operator="<",
                    right_operand=70.0,  # RSIが70未満でエントリー（より緩い条件）
                )
            ]
            ...
            exit_conditions = [
                Condition(
                    left_operand="RSI",
                    operator=">",
                    right_operand=80.0,  # RSIが80超でイグジット
                )
            ]

            strategy_gene = StrategyGene(
                id="debug_strategy_001",
            ...
            for i, ind in enumerate(strategy_gene.indicators):
                print(
                    f"  指標{i+1}: {ind.type}, enabled={ind.enabled}, params={ind.parameters}"
                )
            ...
            # バックテスト設定
            config = {
                "strategy_name": "Debug_Condition_Strategy",
                "symbol": "BTC/USDT:USDT",
                "timeframe": "1h",
                "start_date": datetime.now() - timedelta(days=7),  # 短期間でテスト
                "end_date": datetime.now() - timedelta(days=1),
                "initial_capital": 10000000.0,  # 1000万円に増加
