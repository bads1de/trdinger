...
def create_performance_metrics(
    ...
    return {
        ...
        'long_pnl': long_pnl,
        'short_pnl': short_pnl,
        'long_short_ratio': short_trades / (long_trades + short_trades) if (long_trades + short_trades) > 0 else 0,
        'profit_balance': min(long_pnl, short_pnl) / max(abs(long_pnl), abs(short_pnl)) if max(abs(long_pnl), abs(short_pnl)) > 0 else 0
    }
...
def simulate_legacy_system_performance() -> List[Dict[str, Any]]:
    ...
    for i in range(20):  # 20回の戦略生成をシミュレート
        ...
        performance = create_performance_metrics(
            ...
            long_trades + short_trades, long_trades, short_trades,
            long_pnl, short_pnl
        )
...
def test_performance_comparison():
    ...
    try:
        ...
        metrics = ['total_return', 'sharpe_ratio', 'max_drawdown', 'win_rate', 
                  'long_short_ratio', 'profit_balance']
        ...
        for metric, results in comparison_results.items():
            print(f"{metric:20s}: {results['legacy']:.4f} -> {results['enhanced']:.4f} "
...
def test_strategy_diversity_improvement():
    ...
    try:
        ...
        strategy_patterns = [
            ...
            'short_specialized_ML_DOWN_PROB-ATR',
...
def test_ml_indicator_impact():
    ...
    try:
        ...
        for metric in metrics:
            ...
            print(f"   {metric}: {non_ml_avg:.3f} -> {ml_avg:.3f} ({improvement:+.1f}%)")
...
def test_fitness_sharing_effect():
    ...
    try:
        ...
        # 戦略タイプの分布を比較
        without_similar_ratio = len([s for s in without_sharing if 'similar' in s['strategy_type']]) / len(without_sharing)
        with_similar_ratio = len([s for s in with_sharing if 'similar' in s['strategy_type']]) / len(with_sharing)
        ...
        print(f"   Diversity improvement: {(1-with_similar_ratio) - (1-without_similar_ratio):.1%}")
