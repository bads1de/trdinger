    ...
    try:
        ...
        if count > 0:
            ...
            for record in records:
                print(
                    f"  ID: {record[0]}, Name: {record[1]}, Model: {record[2]}, Score: {record[3]}"
                )
                print(
                    f"      Created: {record[4]}, SaveAsProfile: {record[5]}, TargetModel: {record[6]}"
                )
        else:
        ...
        if response.status_code == 200:
            ...
            if result.get("success") and "result" in result:
                ...
                if "saved_profile_id" in api_result:
                    print(
                        f"✅ 保存されたプロファイルID: {api_result['saved_profile_id']}"
                    )
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if result.get("success") and "result" in result:
                ...
                if "saved_profile_id" in api_result:
                    print(
                        f"✅ バックエンド直接: 保存されたプロファイルID: {api_result['saved_profile_id']}"
                    )
                    return True
                else:
...
def main():
    ...
    print("\n=== テスト結果サマリー ===")
    print(
        f"フロントエンド（プロファイル保存あり）: {'✅ 成功' if frontend_with_profile else '❌ 失敗'}"
    )
    print(
        f"フロントエンド（プロファイル保存なし）: {'✅ 成功' if frontend_without_profile else '❌ 失敗'}"
    )
