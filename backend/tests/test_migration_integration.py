...
class TestMigrationIntegration:
    ...
    def test_all_supported_indicators_with_real_data(self):
        ...
        for indicator_name in supported_indicators:
            try:
                ...
                # 基本的な検証
                self._validate_indicator_result(
                    result, indicator_name, len(self.real_data)
                )
    ...
    def test_ml_feature_engineering_integration(self):
        ...
        try:
            ...
            assert len(tech_features) == len(
                self.real_data
            ), "TechnicalFeatureEngineer: 長さが一致しません"
            assert (
                not tech_features.empty
            ), "TechnicalFeatureEngineer: 空のDataFrameです"

            # NaN の割合をチェック
            nan_ratio = tech_features.isna().sum().sum() / (
                tech_features.shape[0] * tech_features.shape[1]
            )
            assert (
                nan_ratio < 0.5
        ...
        try:
            ...
            assert len(adv_features) == len(
                self.real_data
            ), "AdvancedFeatureEngineer: 長さが一致しません"
            assert not adv_features.empty, "AdvancedFeatureEngineer: 空のDataFrameです"

            # NaN の割合をチェック
            nan_ratio = adv_features.isna().sum().sum() / (
                adv_features.shape[0] * adv_features.shape[1]
            )
            assert (
                nan_ratio < 0.5
    ...
    def test_concurrent_calculation(self):
        ...
        for indicator_name, params in test_cases:
            thread = threading.Thread(
                target=calculate_indicator, args=(indicator_name, params)
            )
    ...
    def _validate_indicator_result(
        self, result: Any, indicator_name: str, expected_length: int
    ):
        """指標結果の基本検証"""
        ...
        if isinstance(result, tuple):
            for i, arr in enumerate(result):
                assert (
                    len(arr) == expected_length
                ), f"{indicator_name}: 結果の長さが不正 (index {i})"
                assert isinstance(
                    arr, np.ndarray
                ), f"{indicator_name}: numpy配列が期待されます (index {i})"
        else:
            assert len(result) == expected_length, f"{indicator_name}: 結果の長さが不正"
            assert isinstance(
                result, np.ndarray
            ), f"{indicator_name}: numpy配列が期待されます"
