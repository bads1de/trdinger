...
class IndicatorGene:
    ...
    def validate(self) -> bool:
        ...
        valid_indicator_types = [
            ...
            "TRANGE", "OBV", "AD", "ADOSC", "TEMA", "DEMA", "T3", "WMA", "KAMA",
        ]
...
class Condition:
    ...
    def validate(self) -> bool:
        ...
        if isinstance(self.left_operand, str):
            if not (self._is_indicator_name(self.left_operand) or 
                   self.left_operand in valid_data_sources):
                return False
                
        if isinstance(self.right_operand, str):
            if not (self._is_indicator_name(self.right_operand) or 
                   self.right_operand in valid_data_sources):
                return False
...
def test_corrected_models():
    ...
    try:
        ...
        valid_conditions = [
            ...
            Condition(left_operand="FundingRate", operator=">", right_operand=0.001),  # 判断材料
            Condition(left_operand="OpenInterest", operator=">", right_operand=1000000),  # 判断材料
            Condition(left_operand="close", operator="cross_above", right_operand="SMA_20"),
        ]
        
        for i, condition in enumerate(valid_conditions):
            if condition.validate():
                print(f"  ✅ 条件{i+1}: {condition.left_operand} {condition.operator} {condition.right_operand}")
            else:
                print(f"  ❌ 条件{i+1}: 無効")
                return False
        ...
        oi_fr_indicators = [ind for ind in gene.indicators 
                           if ind.type in ["OpenInterest", "FundingRate"] or 
                              ind.type.startswith(("OI_", "FR_"))]
        
        if not oi_fr_indicators:
            print("  ✅ 指標: OI/FRを指標として使用していない (正しい)")
        else:
            print(f"  ❌ 指標: OI/FRを指標として使用している: {[ind.type for ind in oi_fr_indicators]}")
            return False
