...
class TestMLOrchestratorDynamicParams:
    ...
    def create_sample_ohlcv_data(
        self, symbol_hint=None, timeframe_minutes=60, num_rows=100
    ):
        """サンプルOHLCVデータを作成"""
        # 時間インデックスを作成
        start_time = datetime.now() - timedelta(hours=num_rows)
        time_index = pd.date_range(
            start=start_time, periods=num_rows, freq=f"{timeframe_minutes}min"
        )
    ...
    def test_infer_timeframe_default(self):
        ...
        non_datetime_df = pd.DataFrame({"Close": [100, 200, 300]})
    ...
    def test_symbol_inference_with_column_names(self):
        ...
        # BTCを含むカラム名
        df = pd.DataFrame(
            {"BTC_Close": [45000, 46000, 47000], "Volume": [1000, 1100, 1200]}
        )
        symbol = self.ml_orchestrator._infer_symbol_from_data(df)
        assert symbol == "BTC/USDT:USDT"
        ...
        # ETHを含むカラム名
        df = pd.DataFrame(
            {"ETH_Price": [2500, 2600, 2700], "Volume": [1000, 1100, 1200]}
        )
        symbol = self.ml_orchestrator._infer_symbol_from_data(df)
        assert symbol == "ETH/USDT:USDT"
