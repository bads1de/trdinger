...
class TestDataOrchestrationComprehensive:
    ...
    async def test_funding_rate_orchestration(self, mock_db):
        ...
        mock_service.fetch_funding_rate_history = AsyncMock(return_value=[{"timestamp": 1000, "rate": 0.01}])
        mock_service.fetch_all_funding_rate_history = AsyncMock(return_value=[{"timestamp": 1000, "rate": 0.01}])
        ...
        with patch("app.services.data_collection.orchestration.funding_rate_orchestration_service.FundingRateRepository") as MockRepo:
            ...
            result = await service.collect_funding_rate_data("BTCUSDT", limit=100, fetch_all=True, db_session=mock_db)
    ...
    async def test_open_interest_orchestration(self, mock_db):
        mock_service = MagicMock()
        mock_service.fetch_and_save_open_interest_data = AsyncMock(return_value={"success": True, "saved_count": 10})
    ...
    async def test_long_short_ratio_orchestration(self, mock_db):
        mock_service = MagicMock()
        mock_service.collect_historical_long_short_ratio_data = AsyncMock(return_value=50)
        
        service = LongShortRatioOrchestrationService(bybit_service=mock_service)
        result = await service.collect_long_short_ratio_data("BTCUSDT", period="1h", fetch_all=True, db_session=mock_db)
    ...
    async def test_start_historical_collection(self, mock_db, mock_bg_tasks):
        ...
        with patch.object(service, "validate_symbol_and_timeframe", return_value="BTC/USDT:USDT"):
            with patch("app.services.data_collection.orchestration.data_collection_orchestration_service.OHLCVRepository") as MockRepo:
                MockRepo.return_value.get_data_count.return_value = 0
                
                resp = await service.start_historical_data_collection("BTC/USDT:USDT", "1h", mock_bg_tasks, mock_db)
    ...
    async def test_bulk_incremental_update(self, mock_db):
        service = DataCollectionOrchestrationService()
        with patch.object(service.historical_service, "collect_bulk_incremental_data", AsyncMock(return_value={"ohlcv": 10})):
