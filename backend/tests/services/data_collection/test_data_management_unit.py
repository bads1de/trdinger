...
class TestDataManagementOrchestrationUnit:
    ...
    async def test_reset_all_data_success(self, service, mock_session):
        ...
        with (
            patch(
                "app.services.data_collection.orchestration.data_management_orchestration_service.OHLCVRepository"
            ) as mock_ohlcv_cls,
            patch(
                "app.services.data_collection.orchestration.data_management_orchestration_service.FundingRateRepository"
            ) as mock_fr_cls,
            patch(
    ...
    async def test_reset_data_by_symbol_partial_fail(self, service, mock_session):
        ...
        with (
            ...
            mock_oi_cls.return_value.clear_open_interest_data_by_symbol.return_value = (
                20
            )
    ...
    async def test_get_data_status_success(self, service, mock_session):
        ...
        with (
            ...
            mock_fr_cls.return_value.get_latest_funding_timestamp.return_value = (
                datetime(2023, 1, 1)
            )
            mock_oi_cls.return_value.get_latest_open_interest_timestamp.return_value = (
                datetime(2023, 1, 1)
            )
