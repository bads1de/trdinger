...
class TestOpenInterestOrchestrationUnit:
    ...
    async def test_collect_open_interest_data_success(
        self, service, mock_bybit_service, mock_db
    ):
        # 正常系: データ収集成功
        mock_bybit_service.fetch_and_save_open_interest_data = AsyncMock(
        ...
        with patch(
            ...
            resp = await service.collect_open_interest_data(
                "BTC/USDT:USDT", fetch_all=True, db_session=mock_db
            )
    ...
    async def test_collect_open_interest_data_failure(
        ...
        # 異常系: サービスレベルの失敗
        mock_bybit_service.fetch_and_save_open_interest_data = AsyncMock(
            return_value={"success": False, "error": "API Error", "saved_count": 0}
        )
        ...
        with patch(
            ...
            resp = await service.collect_open_interest_data(
                "BTC/USDT:USDT", db_session=mock_db
            )
    ...
    @pytest.mark.asyncio
    async def test_collect_bulk_open_interest_data(
        self, service, mock_bybit_service, mock_db
    ):
        # 一括収集のテスト (一部成功、一部失敗)
        ...
        with patch(
            ...
            resp = await service.collect_bulk_open_interest_data(
                symbols, db_session=mock_db
            )
    ...
    async def test_orchestration_exception_handling(self, service, mock_db):
        # 予期せぬ例外のハンドリング
        with patch.object(
            service, "_get_db_session", side_effect=Exception("Critical DB Fail")
        ):
