...
def increasing_prices():
    """単調増加する価格データ"""
    return pd.Series([100.0, 101.0, 102.0, 103.0, 104.0, 105.0, 106.0, 107.0, 108.0, 109.0, 110.0])


@pytest.fixture
def decreasing_prices():
    """単調減少する価格データ"""
    return pd.Series([110.0, 109.0, 108.0, 107.0, 106.0, 105.0, 104.0, 103.0, 102.0, 101.0, 100.0])
...
class TestRSICalculationAccuracy:
    ...
    def test_rsi_all_gains(self):
        ...
        assert valid_values.iloc[-1] > 90, f"全て上昇の場合RSIは90以上であるべき: {valid_values.iloc[-1]}"

    def test_rsi_all_losses(self):
        """全て下落の場合、RSIは0に近づくことを検証"""
        # 全て1ずつ下落
        prices = pd.Series([115.0, 114.0, 113.0, 112.0, 111.0, 110.0, 109.0, 108.0, 107.0, 106.0, 105.0, 104.0, 103.0, 102.0, 101.0, 100.0])
        result = MomentumIndicators.rsi(prices, period=14)

        valid_values = result.dropna()
        assert len(valid_values) > 0, "RSIの有効な値がありません"
        # 全て下落なのでRSIは0に近い値になる
        assert valid_values.iloc[-1] < 10, f"全て下落の場合RSIは10以下であるべき: {valid_values.iloc[-1]}"
...
class TestMACDCalculationAccuracy:
    ...
    def test_macd_line_is_fast_ema_minus_slow_ema(self, known_prices):
        ...
        for i in range(slow - 1, len(known_prices)):
            if not np.isnan(macd_line.iloc[i]) and not np.isnan(ema_fast.iloc[i]) and not np.isnan(ema_slow.iloc[i]):
...
class TestBollingerBandsAccuracy:
    ...
    def test_bbands_order(self, known_prices):
        ...
        assert (upper[valid] >= middle[valid] - 1e-10).all(), "upper >= middle が成り立たない"
        assert (middle[valid] >= lower[valid] - 1e-10).all(), "middle >= lower が成り立たない"
...
class TestVWAPCalculationAccuracy:
    ...
    def test_vwap_constant_prices(self):
        ...
        vwap = VolumeIndicators.vwap(data["high"], data["low"], data["close"], data["volume"])
...
class TestStochasticAccuracy:
    ...
    def test_stochastic_range(self, known_ohlcv):
        ...
        k, d = MomentumIndicators.stoch(
            known_ohlcv["high"], known_ohlcv["low"], known_ohlcv["close"],
            k=3, d=2, smooth_k=1
        )
        ...
        assert (valid_k >= 0).all() and (valid_k <= 100).all(), f"%Kが範囲外: {valid_k.min()}-{valid_k.max()}"
        assert (valid_d >= 0).all() and (valid_d <= 100).all(), f"%Dが範囲外: {valid_d.min()}-{valid_d.max()}"
...
class TestIndicatorEdgeCases:
    ...
    def test_rsi_minimum_data(self):
        ...
        data = pd.Series([100.0, 101.0, 102.0, 103.0, 104.0, 105.0, 106.0, 107.0, 108.0, 109.0, 110.0, 111.0, 112.0, 113.0, 114.0, 115.0])
    ...
    def test_atr_no_volatility(self):
        ...
        atr = VolatilityIndicators.atr(data["high"], data["low"], data["close"], length=5)
...
class TestIndicatorConsistency:
    ...
    def test_bb_width_increases_with_std_multiplier(self, known_prices):
        ...
        assert (width2[valid] > width1[valid]).all(), "std倍数が大きいほどバンド幅は広がるべき"
