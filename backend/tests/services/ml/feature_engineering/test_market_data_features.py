...
class TestMarketDataFeatureCalculator:
    ...
    def sample_fr(self):
        dates = pd.date_range(start="2023-01-01", periods=200, freq="h")
        return pd.DataFrame(
            {"timestamp": dates, "funding_rate": [0.0001] * 100 + [0.0005] * 100}
        )
    ...
    def sample_oi(self):
        dates = pd.date_range(start="2023-01-01", periods=200, freq="h")
        return pd.DataFrame(
            {"timestamp": dates, "open_interest": np.linspace(1000000, 2000000, 200)}
        )

    def test_calculate_features_all(self, sample_ohlcv, sample_fr, sample_oi):
        """FRとOIの両方がある場合の計算"""
        calc = MarketDataFeatureCalculator()
        config = {"funding_rate_data": sample_fr, "open_interest_data": sample_oi}
    ...
    def test_timezone_alignment(self, sample_ohlcv, sample_fr):
        ...
        res, col = calc._process_market_data(
            sample_ohlcv, sample_fr, ["funding_rate"], "_fr"
        )
    ...
    def test_missing_columns_fallback(self, sample_ohlcv):
        ...
        calc = MarketDataFeatureCalculator()
        bad_fr = pd.DataFrame(
            {"timestamp": sample_ohlcv.index, "wrong_col": [0.1] * 200}
        )
