...
from app.services.ml.feature_engineering.oi_fr_interaction_features import (
...
class TestOIFRInteractionFeatureCalculator:
    ...
    def sample_oi_fr(self, sample_ohlcv):
        ...
        oi_data = pd.DataFrame(
            {"open_interest": np.random.uniform(1e8, 2e8, n)}, index=sample_ohlcv.index
        )
    ...
    def test_calculate_features_basic(self, sample_ohlcv, sample_oi_fr):
        ...
        features = calculator.calculate_features(
            df=sample_ohlcv, oi_data=oi_data, fr_data=fr_data
        )
    ...
    def test_calculate_features_missing_data(self, sample_ohlcv):
        ...
        # OI/FRがNoneの場合
        features = calculator.calculate_features(
            sample_ohlcv, oi_data=None, fr_data=None
        )
        ...
        features = calculator.calculate_features(
            sample_ohlcv, oi_data=oi_data, fr_data=None
        )
        assert features.empty
    ...
    def test_unaligned_index(self, sample_ohlcv, sample_oi_fr):
        ...
        oi_unaligned.index = pd.date_range(
            "2025-01-01", periods=len(oi_data), freq="1h"
        )

        features = calculator.calculate_features(
            sample_ohlcv, oi_data=oi_unaligned, fr_data=fr_data
        )
        assert features.empty
    ...
    def test_zero_values_handling(self, sample_ohlcv):
        ...
        calculator = OIFRInteractionFeatureCalculator()
        features = calculator.calculate_features(
            sample_ohlcv, oi_data=oi_data, fr_data=fr_data
        )
