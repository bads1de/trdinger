...
class TestDataFrequencyManager:
    ...
    def test_align_data_frequencies_upsampling(self, manager):
        ...
        oi_df = pd.DataFrame(
            {"open_interest": [100, 110, 120, 130, 140, 150]}, index=oi_ts
        )
    ...
    def test_align_data_frequencies_downsampling(self, manager):
        ...
        ohlcv = pd.DataFrame(
            {"close": [10, 11]}, index=pd.date_range("2023-01-01", periods=2, freq="4h")
        )
        ...
        aligned_fr, _ = manager.align_data_frequencies(
            ohlcv, funding_rate_data=fr, ohlcv_timeframe="4h"
        )
    ...
    def test_detect_ohlcv_timeframe_thresholds(self, manager):
        ...
        # 15m (<= 22.5)
        df_15m = pd.DataFrame(
            index=pd.date_range("2023-01-01", periods=2, freq="22.5min")
        )
