...
class TestDataLeakComprehensive:
    ...
    def test_feature_engineering_no_future_leak(self, sample_ohlcv_data):
        ...
        full_features = feature_service.calculate_advanced_features(ohlcv_data=sample_ohlcv_data)

        split_point = int(len(sample_ohlcv_data) * 0.8)
        partial_data = sample_ohlcv_data.iloc[:split_point]
        partial_features = feature_service.calculate_advanced_features(ohlcv_data=partial_data)
    ...
    def test_scaler_fit_on_train_only(self, mock_trainer):
        ...
        X_train = pd.DataFrame(np.random.randn(100, 5) * 10, columns=[f"f{i}" for i in range(5)])
        X_test = pd.DataFrame(np.random.randn(20, 5) * 10 + 50, columns=[f"f{i}" for i in range(5)])
        ...
        np.testing.assert_allclose(mock_trainer.scaler.mean_, X_train.mean().values, rtol=1e-10)
