...
class TestStackingEnsemble:
    ...
    def test_create_cv_splitter(self, config, sample_data):
        ...
        assert isinstance(ensemble._create_cv_splitter(X), pd.get_option("compute.use_bottleneck") and None or __import__('sklearn.model_selection').model_selection.KFold)
        
        # 2. StratifiedKFold
        ensemble.config["cv_strategy"] = "stratified_kfold"
        assert isinstance(ensemble._create_cv_splitter(X), __import__('sklearn.model_selection').model_selection.StratifiedKFold)
        
        # 3. PurgedKFold (Default)
        ensemble.config["cv_strategy"] = "purged_kfold"
        with patch('app.services.ml.ensemble.stacking.get_t1_series', return_value=pd.Series()):
            assert isinstance(ensemble._create_cv_splitter(X), __import__('app.services.ml.cross_validation.purged_kfold').services.ml.cross_validation.purged_kfold.PurgedKFold)
    ...
    def test_base_model_creation_failure(self, config):
        ...
        with patch.object(ensemble, '_create_base_model', side_effect=[MagicMock(), ValueError("Fail")]):
    ...
    def test_load_models_legacy_format(self, config, tmp_path):
        """旧形式（StackingClassifier）のモデル読み込み"""
        ensemble = StackingEnsemble(config)
        base_path = str(tmp_path / "legacy_model")
        mock_file = f"{base_path}_stacking_classifier_v1.pkl"
        Path(mock_file).touch()
        ...
        with patch('joblib.load', return_value=model_data):
            with patch('glob.glob', side_effect=[[], [mock_file]]): # 1回目は新形式なし
                success = ensemble.load_models(base_path)
                assert success is True
    ...
    def test_evaluate_ensemble(self, config, sample_data):
        ...
        with patch.object(ensemble, 'predict_proba', return_value=np.zeros((len(X), 2))):
            with patch.object(ensemble, '_evaluate_predictions', return_value={"accuracy": 0.8}):
