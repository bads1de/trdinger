...
class TestVolatilityUtils:
    ...
    def test_calculate_volatility_atr(self, sample_ohlc):
        """ATRベースのボラティリティ"""
        res = calculate_volatility_atr(
            sample_ohlc["high"], sample_ohlc["low"], sample_ohlc["close"], window=10
        )
        assert len(res) == 50
        # h=102, l=98 なら TR=4
        ...
        # %表示
        res_pct = calculate_volatility_atr(
            sample_ohlc["high"],
            sample_ohlc["low"],
            sample_ohlc["close"],
            as_percentage=True,
    ...
    def test_calculate_historical_volatility(self, sample_returns):
        ...
        res = calculate_historical_volatility(
            sample_returns, window=20, annualize=True, periods_per_year=252
        )
        # std * sqrt(252)
    ...
    def test_calculate_realized_volatility(self, sample_returns):
        ...
        res = calculate_realized_volatility(
            sample_returns, window=24, periods_per_day=24
        )
