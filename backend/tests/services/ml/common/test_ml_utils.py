...
class TestMLUtils:
    ...
    def test_generate_cache_key(self):
        """キャッシュキー生成のテスト"""
        df1 = pd.DataFrame(
            {"close": [100, 101]}, index=pd.date_range("2023-01-01", periods=2)
        )
        df2 = pd.DataFrame(
            {"close": [100, 102]}, index=pd.date_range("2023-01-01", periods=2)
        )
    ...
    def test_predict_class_from_proba(self):
        ...
        # 多クラス
        probs_multi = np.array([[0.8, 0.1, 0.1], [0.2, 0.7, 0.1]])
