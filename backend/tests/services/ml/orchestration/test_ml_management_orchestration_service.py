...
class TestMLManagementOrchestrationService:
    ...
    async def test_get_feature_importance(self, service):
        """特徴量重要度の取得"""
        mock_info = {"metadata": {"feature_importance": {"f1": 0.5, "f2": 0.3}}}
        with patch(
            "app.services.ml.orchestration.ml_management_orchestration_service.get_latest_model_with_info",
            ...
            assert "f2" not in res["feature_importance"]
    ...
    async def test_load_model_flow(self, service):
        ...
        with patch(
            ...
            with patch(
                "app.services.ml.orchestration.ml_management_orchestration_service.ml_training_service.load_model",
                return_value=True,
            ):
                with patch.object(
                    service, "get_current_model_info", return_value={"loaded": True}
                ):
    ...
    async def test_get_current_model_info_not_found(self, service):
        ...
        with patch(
            "app.services.ml.orchestration.ml_training_orchestration_service.ml_training_service.get_current_model_path",
            return_value="ghost.pkl",
        ):
            with patch(
                "app.services.ml.orchestration.ml_training_orchestration_service.ml_training_service.get_current_model_info",
                return_value={"model_type": "lgb"},
            ):
    ...
    async def test_update_ml_config_fail(self, service):
        """設定更新失敗時のハンドリング"""
        with patch(
            "app.services.ml.orchestration.ml_management_orchestration_service.ml_config_manager.update_config",
            return_value=False,
        ):
