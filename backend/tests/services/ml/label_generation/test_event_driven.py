...
from app.services.ml.label_generation.event_driven import EventDrivenLabelGenerator
...
class TestEventDrivenLabelGenerator:
    ...
    def test_generate_labels_with_regimes(self, generator, sample_market_data):
        ...
        labels_df, info = generator.generate_hrhp_lrlp_labels(
            sample_market_data, regime_labels=regimes
        )
    ...
    def test_profile_overrides(self, generator, sample_market_data):
        ...
        overrides = {"hrhp": {"base_tp": 0.001, "base_sl": 0.001, "holding_period": 5}}
        labels_df, _ = generator.generate_hrhp_lrlp_labels(
            sample_market_data, profile_overrides=overrides
        )
    ...
    def test_first_touch_logic(self, generator):
        ...
        label = generator._first_touch_label(
            close, high, low, start_idx=0, tp_mult=0.04, sl_mult=0.02, holding=5
        )
        assert label == 1
        ...
        label = generator._first_touch_label(
            close, high, low, start_idx=0, tp_mult=0.05, sl_mult=0.03, holding=5
        )
        assert label == -1
        ...
        label = generator._first_touch_label(
            close, high, low, start_idx=0, tp_mult=0.1, sl_mult=0.1, holding=2
        )
        assert label == 0
    ...
    def test_simultaneous_hit(self, generator):
        ...
        label = generator._first_touch_label(
            close, high, low, start_idx=0, tp_mult=0.05, sl_mult=0.05, holding=1
        )
        assert label == -1
    ...
    def test_zero_price_handling(self, generator):
        ...
        label = generator._first_touch_label(
            close, high, low, start_idx=0, tp_mult=0.1, sl_mult=0.1, holding=1
        )
        assert label == 0
