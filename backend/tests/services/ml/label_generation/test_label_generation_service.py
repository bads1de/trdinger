...
class TestLabelGenerationServiceWithEvents:
    ...
    def test_prepare_labels_uses_training_params_timeframe(
        self, sample_features, sample_ohlcv
    ):
        """学習パラメータの timeframe と horizon_n がラベル生成へ渡ることを確認"""
        service = LabelGenerationService()
        event_index = sample_features.index[:3]
        mock_labels = pd.Series([1, 0, 1], index=event_index, dtype=int)

        with patch(
            "app.services.ml.label_generation.label_generation_service.LabelCache"
        ) as mock_label_cache_cls:
            mock_label_cache = mock_label_cache_cls.return_value
            mock_label_cache.get_labels.return_value = mock_labels

            with patch.object(
                service, "_detect_events", return_value=event_index
            ) as mock_detect_events:
                features_clean, labels_clean = service.prepare_labels(
                    features_df=sample_features,
                    ohlcv_df=sample_ohlcv,
                    threshold_method="TRIPLE_BARRIER",
        ...
        mock_label_cache_cls.assert_called_once_with(sample_ohlcv)
        mock_detect_events.assert_called_once()
        mock_label_cache.get_labels.assert_called_once()
        kwargs = mock_label_cache.get_labels.call_args.kwargs
        assert kwargs["timeframe"] == "4h"
        assert kwargs["horizon_n"] == 12
        assert len(features_clean) == 3
