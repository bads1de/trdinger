...
class TestUniversalStrategyAll:
    ...
    def valid_gene(self):
        return StrategyGene(
            ...
            short_entry_conditions=[Condition(left_operand="close", operator="<", right_operand="sma")],
            risk_management={},
            metadata={"test": True},
        )
    ...
    def test_init_and_indicators(self, mock_broker, mock_data, valid_gene):
        ...
        with patch("app.services.auto_strategy.strategies.universal_strategy.IndicatorCalculator") as MockCalc:
            strategy = UniversalStrategy(mock_broker, mock_data, params)
            strategy.init()
            assert MockCalc.return_value.init_indicator.call_count == len(valid_gene.indicators)

    def test_entry_logic(self, mock_broker, mock_data, valid_gene):
        params = {"strategy_gene": valid_gene}
        with patch("app.services.auto_strategy.strategies.universal_strategy.ConditionEvaluator") as MockEval:
            strategy = UniversalStrategy(mock_broker, mock_data, params)
            strategy.buy = MagicMock()
            with patch.object(UniversalStrategy, "position", new_callable=PropertyMock) as mock_pos:
                mock_pos.return_value = None
                MockEval.return_value.evaluate_conditions.side_effect = [True, False] # Long: True
                strategy.next()
    ...
    def test_tpsl_data_slice_size(self, mock_broker, mock_data_large, valid_gene):
        ...
        valid_gene.long_tpsl_gene = TPSLGene(enabled=True, method=TPSLMethod.VOLATILITY_BASED, atr_period=50)
        strategy = UniversalStrategy(mock_broker, mock_data_large, {"strategy_gene": valid_gene})
        
        with patch.object(strategy.tpsl_service, "calculate_tpsl_prices", return_value=(90.0, 110.0)) as mock_tpsl:
            with patch.object(strategy.condition_evaluator, "evaluate_conditions", return_value=True):
                with patch.object(strategy, "buy"), patch.object(UniversalStrategy, "position", new_callable=PropertyMock) as mock_pos:
                    mock_pos.return_value = None
                    strategy.next()
    ...
    def test_position_size_recalculation(self, mock_broker, mock_data, valid_gene):
        ...
        valid_gene.position_sizing_gene = PositionSizingGene(enabled=True, method=PositionSizingMethod.FIXED_RATIO)
        with patch("app.services.auto_strategy.strategies.universal_strategy.PositionSizingService") as MockPS:
            MockPS.return_value.calculate_position_size_fast.side_effect = [0.05, 0.08]
            strategy = UniversalStrategy(mock_broker, mock_data, {"strategy_gene": valid_gene})
    ...
    def test_stateful_condition_direction(self, mock_broker, mock_data):
        ...
        stateful_short = StatefulCondition(trigger_condition=trigger, follow_condition=follow, lookback_bars=5, direction="short")
        ...
        with patch.object(strategy.condition_evaluator, "evaluate_stateful_condition", return_value=True):
