...
def generate_report(test_results):
    ...
    if passed_tests == total_tests:
        report_lines.append("✅ 全てのテストが成功しました。システムは正常に動作しています。")
        report_lines.append("")
...
def main():
    ...
    test_files = {
        ...
        "基本アダプター機能": "backend/tests/test_basic_adapter_functionality.py",
        "包括的指標テスト": "backend/tests/test_comprehensive_indicators.py",
    ...
    for test_name, test_file in test_files.items():
        if os.path.exists(test_file):
            success, execution_time, stdout, stderr = run_test_file(test_file, test_name)
