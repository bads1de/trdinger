...
def test_extended_strategy_gene():
    ...
    try:
        ...
        entry_conditions = [
            ...
            Condition(left_operand="OpenInterest", operator=">", right_operand="OI_SMA_10"),
            Condition(left_operand="FundingRate", operator="<", right_operand=0.001)
        ]
        
        exit_conditions = [
            Condition(left_operand="close", operator="<", right_operand="SMA_20"),
            Condition(left_operand="FundingRate", operator=">", right_operand="FR_EMA_5")
        ]
        ...
        for i, condition in enumerate(gene.entry_conditions):
            if condition.validate():
                print(f"  ✅ エントリー条件{i}: {condition.left_operand} {condition.operator} {condition.right_operand} - 有効")
            else:
        ...
        for i, condition in enumerate(gene.exit_conditions):
            if condition.validate():
                print(f"  ✅ イグジット条件{i}: {condition.left_operand} {condition.operator} {condition.right_operand} - 有効")
            else:
    ...
    try:
        ...
        for individual in population:
            for indicator in individual.indicators:
                total_indicators += 1
                if any(keyword in indicator.type for keyword in ["OI", "FR", "OpenInterest", "FundingRate"]):
                    oi_fr_count += 1
        
        oi_fr_ratio = oi_fr_count / total_indicators if total_indicators > 0 else 0
        print(f"  📊 OI/FR指標含有率: {oi_fr_ratio:.2%} ({oi_fr_count}/{total_indicators})")
...
def test_strategy_factory_compatibility():
    ...
    try:
        ...
        try:
            ...
            print("  📝 注意: StrategyFactoryがまだOI/FR指標に対応していない可能性があります")
