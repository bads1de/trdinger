...
def sample_config():
    ...
    return {
        ...
        "strategy_config": {
            ...
            "parameters": {"strategy_gene": {"entry_conditions": [], "exit_conditions": []}}
        },
    }
...
def test_run_backtest_success(
    ...
    with patch.object(backtest_service, "run_backtest", return_value=sample_converted_result):
        # 実行
...
def test_run_backtest_data_retrieval_error(backtest_service, sample_config):
    ...
    with patch.object(backtest_service, "run_backtest", side_effect=Exception("Data not found")):
        # 実行とアサーション
...
def test_run_backtest_execution_error(backtest_service, sample_config):
    """バックテスト実行エラー時のテスト"""
    # run_backtestメソッドでエラーを発生させる
    with patch.object(backtest_service, "run_backtest", side_effect=Exception("Execution failed")):
        # 実行とアサーション
...
def test_run_backtest_conversion_error(
    ...
    """結果変換エラー時のテスト"""
    # run_backtestメソッドでエラーを発生させる
    with patch.object(backtest_service, "run_backtest", side_effect=Exception("Conversion failed")):
        # 実行とアサーション
...
def test_execute_and_save_backtest_success(
    ...
    with patch.object(backtest_service, "run_backtest", return_value=sample_converted_result):
        with patch("app.services.backtest.backtest_service.BacktestResultRepository", return_value=mock_backtest_repo):
            mock_backtest_repo.save_backtest_result.return_value = {"id": 1, **sample_converted_result}

            # 実行
            result = backtest_service.execute_and_save_backtest(sample_config, mock_db_session)

            # アサーション
            backtest_service.run_backtest.assert_called_once_with(sample_config)
            mock_backtest_repo.save_backtest_result.assert_called_once_with(sample_converted_result)
...
def test_execute_and_save_backtest_run_error(
    ...
    # モックの設定
    mock_db_session = MagicMock()
    with patch.object(backtest_service, "run_backtest", side_effect=Exception("Run failed")):
        # 実行
        result = backtest_service.execute_and_save_backtest(sample_config, mock_db_session)
