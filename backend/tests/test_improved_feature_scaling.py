...
class TestImprovedFeatureScaling:
    ...
    def test_feature_scaling_improvement(self):
        ...
        if len(ranges) > 1:
            ...
            if scale_ratio < 100:  # 改善前は205万倍だった
                logger.info("✅ 特徴量スケーリングにより、スケール不整合が大幅に改善されました")
            else:
                logger.warning(f"⚠️ スケール不整合が残っています（比率: {scale_ratio:.2f}）")
    ...
    def test_different_scaling_methods(self):
        ...
        for method in scaling_methods:
            ...
            logger.info(f"  平均値範囲: {min([stats[col]['mean'] for col in test_data.columns]):.4f} - {max([stats[col]['mean'] for col in test_data.columns]):.4f}")
    ...
    def test_scaling_with_outlier_removal(self):
        ...
        improvement_ratio = improvement_count / total_features if total_features > 0 else 0
        
        logger.info(f"外れ値除去による安定性改善: {improvement_count}/{total_features} ({improvement_ratio*100:.1f}%)")
...
if __name__ == "__main__":
    ...
    print(f"外れ値除去改善率: {results['outlier_results']['improvement_ratio']*100:.1f}%")
