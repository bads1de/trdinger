...
class TestTechnicalIndicatorService:
    ...
    def test_calculate_indicator_pandas_ta_success(
        self, mock_post_process, mock_call_pandas_ta, mock_validation,
        mock_normalize, mock_get_config, indicator_service, sample_df
    ):
    ...
    def test_calculate_indicator_adapter_fallback(
        self, mock_calculate_adapter, mock_get_indicator_config,
        mock_get_config, indicator_service, sample_df
    ):
        """アダプターフォールバックテスト"""
        mock_config = Mock()
    ...
    def test_calculate_indicator_unsupported(self, indicator_service, sample_df):
        ...
        with patch.object(indicator_service, '_get_config', return_value=None):
            with patch.object(indicator_service, '_get_indicator_config', side_effect=ValueError):
    ...
    def test_basic_validation_success(self, mock_validate_length, indicator_service, sample_df):
        """基本検証成功テスト"""
    ...
    @patch.object(TechnicalIndicatorService, 'validate_data_length_with_fallback')
    def test_basic_validation_data_too_short(self, mock_validate_length, indicator_service, sample_df):
        """データ長不足の検証テスト"""
    ...
    def test_calculate_indicator_error_handling(self, indicator_service, sample_df):
        """指標計算時のエラーハンドリングテスト"""
        with patch.object(indicator_service, '_get_config', side_effect=Exception("Test error")):
