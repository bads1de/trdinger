...
class TestMLManagementAPI:
    ...
    @patch('app.services.ml.orchestration.ml_management_orchestration_service.MLManagementOrchestrationService')
    def test_service_layer_integration(self, mock_service_class):
        ...
        with patch('app.api.dependencies.get_ml_management_orchestration_service', return_value=mock_service):
            # 設定取得テスト
            response = self.client.get("/api/ml/config")
            assert response.status_code == 200
