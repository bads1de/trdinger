...
class TestValidatorsUnit:
    ...
    def test_validate_extended_data_errors(self):
        # funding_rate 範囲外
        with pytest.raises(
            ValueError, match="funding_rate values must be between -1 and 1"
        ):
    ...
    def test_validate_data_integrity_without_timestamp(self):
        df = pd.DataFrame({"close": [100, 101, 102]})
        assert validate_data_integrity(df) is True

    def test_validate_data_integrity_with_timestamp(self):
        df = pd.DataFrame(
            {
