...
class TestTPSLSplit:
    ...
    def test_crossover_handles_split_tpsl(self):
        ...
        long_tpsl1 = TPSLGene(method=TPSLMethod.FIXED_PERCENTAGE, take_profit_pct=0.1, enabled=True)
        short_tpsl1 = TPSLGene(method=TPSLMethod.FIXED_PERCENTAGE, take_profit_pct=0.1, enabled=True)
        
        long_tpsl2 = TPSLGene(method=TPSLMethod.FIXED_PERCENTAGE, take_profit_pct=0.2, enabled=True)
        short_tpsl2 = TPSLGene(method=TPSLMethod.FIXED_PERCENTAGE, take_profit_pct=0.2, enabled=True)

        parent1 = StrategyGene(
            long_tpsl_gene=long_tpsl1,
            short_tpsl_gene=short_tpsl1
        )
        parent2 = StrategyGene(
            long_tpsl_gene=long_tpsl2,
            short_tpsl_gene=short_tpsl2
        )

        # 交叉実行
        child1, child2 = crossover_strategy_genes_pure(parent1, parent2, config) # configを渡す
    ...
    def test_mutation_affects_split_tpsl(self):
        ...
        # 変異率1.0で確実に変異させる
        original_tp = 0.1
        long_tpsl = TPSLGene(method=TPSLMethod.FIXED_PERCENTAGE, take_profit_pct=original_tp, enabled=True)
        short_tpsl = TPSLGene(method=TPSLMethod.FIXED_PERCENTAGE, take_profit_pct=original_tp, enabled=True)
        ...
        mutated = mutate_strategy_gene_pure(gene, config, mutation_rate=1.0) # configを渡す
