...
from app.services.auto_strategy.generators.strategies.complex_conditions_strategy import ComplexConditionsStrategy
...
class TestComplexConditionsStrategy(unittest.TestCase):
    ...
    def test_generate_trend_pullback_pattern(self):
        ...
        sma = IndicatorGene(type="SMA", parameters={"length": 50}, enabled=True, id="sma_50")
        rsi = IndicatorGene(type="RSI", parameters={"length": 14}, enabled=True, id="rsi_14")
        ...
        def check_cond(cond):
            ...
            if isinstance(cond, ConditionGroup):
                ...
                if str(cond.left_operand).lower() == "close" and "SMA" in str(cond.right_operand):
                    has_sma_condition = True
    ...
    def test_generate_moving_average_cross_pattern(self):
        """移動平均線クロスパターンの生成テスト"""
        # 短期SMAと長期SMAを用意
        sma_short = IndicatorGene(type="SMA", parameters={"length": 10}, enabled=True, id="sma_10")
        sma_long = IndicatorGene(type="SMA", parameters={"length": 50}, enabled=True, id="sma_50")
        
        # 条件生成
        long_result, short_result, _ = self.strategy.generate_conditions([sma_short, sma_long])
    ...
    def test_generate_volatility_breakout_pattern(self):
        ...
        # BBを用意
        bb = IndicatorGene(type="BB", parameters={"length": 20, "std": 2.0}, enabled=True, id="bb_20")
        ...
        def check_cond(cond):
            ...
            if isinstance(cond, Condition):
                ...
                if str(cond.left_operand).lower() == "close" and "BB" in str(cond.right_operand):
                    has_breakout = True
