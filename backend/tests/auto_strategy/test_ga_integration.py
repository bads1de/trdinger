...
def test_ga_config_with_new_tpsl():
    ...
    try:
        # 直接インポート
        sys.path.append(os.path.join(os.path.dirname(__file__), 'app', 'core', 'services', 'auto_strategy', 'models'))
...
def test_legacy_compatibility():
    ...
    try:
        ...
        ga_config_legacy = GAConfig(
            ...
            position_size_range=[0.1, 0.5]
        )
        
        print("✅ 従来方式GA設定作成成功")
        print(f"   - TP/SL戦略: {ga_config_legacy.tpsl_strategy}")
        print(f"   - SL範囲: {ga_config_legacy.stop_loss_range[0]:.1%} - {ga_config_legacy.stop_loss_range[1]:.1%}")
        print(f"   - TP範囲: {ga_config_legacy.take_profit_range[0]:.1%} - {ga_config_legacy.take_profit_range[1]:.1%}")
...
def test_strategy_factory_integration():
    ...
    try:
        # 直接インポート
        sys.path.append(os.path.join(os.path.dirname(__file__), 'app', 'core', 'services', 'auto_strategy', 'factories'))
        sys.path.append(os.path.join(os.path.dirname(__file__), 'app', 'core', 'services', 'auto_strategy', 'models'))
        ...
        print(f"   - SL価格: ${sl_price_adv:,.0f} ({((current_price - sl_price_adv) / current_price * 100):.1f}%)")
        print(f"   - TP価格: ${tp_price_adv:,.0f} ({((tp_price_adv - current_price) / current_price * 100):.1f}%)")
        ...
        print("✅ 従来方式TP/SL価格計算:")
        print(f"   - SL価格: ${sl_price_leg:,.0f} ({((current_price - sl_price_leg) / current_price * 100):.1f}%)")
        print(f"   - TP価格: ${tp_price_leg:,.0f} ({((tp_price_leg - current_price) / current_price * 100):.1f}%)")
...
def test_end_to_end_workflow():
    ...
    try:
        ...
        from tpsl_auto_decision_service import TPSLAutoDecisionService, TPSLConfig, TPSLStrategy
