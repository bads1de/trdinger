...
class TestStrategyGene:
    ...
    def test_adaptive_mutate(self, sample_gene, mock_config):
        ...
        mutated = sample_gene.adaptive_mutate(population, mock_config, base_mutation_rate=0.5)
        assert mutated.metadata["adaptive_mutation_rate"] < 0.5
        
        # 分散が小さい場合 -> 変異率上昇
        mock_config.adaptive_mutation_variance_threshold = 100.0
        mutated = sample_gene.adaptive_mutate(population, mock_config, base_mutation_rate=0.1)
    ...
    def test_properties(self, sample_gene):
        ...
        # position_sizing_geneがない場合のデフォルト
        sample_gene.position_sizing_gene = None
        assert sample_gene.method == PositionSizingMethod.FIXED_RATIO
        
        # ある場合
        sample_gene.position_sizing_gene = MagicMock(method=PositionSizingMethod.FIXED_QUANTITY)
        assert sample_gene.method == PositionSizingMethod.FIXED_QUANTITY
