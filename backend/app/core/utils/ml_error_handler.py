...
class MLErrorHandler:
        ...
        **kwargs
        ...
        try:
            ...
            if "timeout" in str(e).lower():
                raise MLTimeoutError(f"処理がタイムアウトしました（{timeout_seconds}秒）: {e}")
            raise
        ...
        **kwargs
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=1) as executor:
            ...
            try:
                ...
                raise MLTimeoutError(f"Windows環境でのタイムアウト（{timeout_seconds}秒）")
    ...
    def validate_dataframe(
        ...
        if len(df) < min_rows:
            raise MLDataError(f"データ行数が不足しています（必要: {min_rows}行, 実際: {len(df)}行）")
    ...
    def validate_predictions(predictions: Dict[str, float]) -> bool:
        ...
        if missing_keys:
            raise MLValidationError(f"予測結果に必要なキーが不足しています: {missing_keys}")
        ...
        for key in required_keys:
            ...
            if not (0 <= value <= 1):
                raise MLValidationError(f"予測値が範囲外です: {key}={value} (0-1の範囲である必要があります)")
        
        # 合計値の確認
        total = sum(predictions[key] for key in required_keys)
        if not (0.8 <= total <= 1.2):
            raise MLValidationError(f"予測値の合計が範囲外です: {total} (0.8-1.2の範囲である必要があります)")
    ...
    def log_memory_usage(operation_name: str, threshold_mb: int = 1000):
        ...
        try:
            ...
            if memory_mb > threshold_mb:
                logger.warning(f"{operation_name}: メモリ使用量が閾値を超過 ({memory_mb:.1f}MB > {threshold_mb}MB)")
            else:
                logger.debug(f"{operation_name}: メモリ使用量 {memory_mb:.1f}MB")
        except ImportError:
            logger.debug(f"{operation_name}: psutilが利用できないため、メモリ監視をスキップ")
        except Exception as e:
...
def ml_operation_context(operation_name: str, log_memory: bool = True):
    ...
    try:
        ...
        duration = time.time() - start_time
        logger.error(f"{operation_name} でエラーが発生しました（{duration:.2f}秒）: {e}")
        raise
    finally:
        if log_memory:
