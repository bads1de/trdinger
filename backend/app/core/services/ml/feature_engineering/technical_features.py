...
class TechnicalFeatureCalculator:
    ...
    def calculate_market_regime_features(
        ...
        try:
            ...
            result_df['Market_Efficiency'] = (
                returns.rolling(window=volatility_period).apply(
    ...
    def calculate_pattern_features(
        ...
        try:
            ...
            # ダイバージェンス検出
            rsi = result_df['RSI'] if 'RSI' in result_df.columns else self._calculate_rsi(result_df)
            
            # 価格とRSIのダイバージェンス
            price_trend = result_df['Close'].rolling(window=10).apply(
            ...
            result_df['Bear_Divergence'] = (
                (price_trend > 0) & (rsi_trend < 0)
            ).astype(int)
            
            # ブルダイバージェンス（価格下降、RSI上昇）
            result_df['Bull_Divergence'] = (
                (price_trend < 0) & (rsi_trend > 0)
            ).astype(int)
            ...
            for level in fib_levels:
                ...
                result_df[f'Fib_{int(level*1000)}_Distance'] = abs(
                    result_df['Close'] - fib_price
                ) / result_df['Close']
    ...
    def get_feature_names(self) -> list:
        ...
        return [
            ...
            'Fib_236_Distance', 'Fib_382_Distance', 'Fib_500_Distance',
            'Fib_618_Distance', 'Fib_786_Distance',
