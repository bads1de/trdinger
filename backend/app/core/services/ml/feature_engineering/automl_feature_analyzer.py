...
class AutoMLFeatureAnalyzer:
    ...
    def _analyze_single_feature(self, feature_name: str, importance: float) -> FeatureAnalysis:
        """単一特徴量を分析"""
        feature_type = self._identify_feature_type(feature_name)
        category = self._categorize_feature(feature_name, feature_type)
        description = self._generate_feature_description(feature_name, feature_type, category)
    ...
    def _identify_feature_type(self, feature_name: str) -> str:
        ...
        # 手動特徴量の判定
        manual_patterns = [
            r'.*_sma_.*', r'.*_ema_.*', r'.*_rsi.*', r'.*_macd.*',
            r'.*_bb_.*', r'.*_volume_.*', r'.*_price_.*'
        ]
    ...
    def _categorize_feature(self, feature_name: str, feature_type: str) -> str:
        ...
        # 周波数領域特徴量
        if any(freq in name_lower for freq in ['fft', 'frequency', 'spectral', 'energy']):
            return 'frequency'
    ...
    def _generate_feature_description(self, feature_name: str, feature_type: str, category: str) -> str:
        """特徴量の説明を生成"""
    ...
    def _calculate_type_statistics(self, features: List[FeatureAnalysis]) -> Dict[str, Any]:
        """タイプ別統計を計算"""
        type_counts = {}
        type_importance = {}
    ...
    def _calculate_category_statistics(self, features: List[FeatureAnalysis]) -> Dict[str, Any]:
        """カテゴリ別統計を計算"""
        category_counts = {}
        category_importance = {}
    ...
    def _analyze_automl_impact(self, features: List[FeatureAnalysis]) -> Dict[str, Any]:
        ...
        return {
            ...
            "manual_importance_ratio": (manual_importance / total_importance) * 100 if total_importance > 0 else 0,
            "automl_importance_ratio": (automl_importance / total_importance) * 100 if total_importance > 0 else 0,
            ...
            "top_automl_features": [
                ...
                for f in sorted(automl_features, key=lambda x: x.importance, reverse=True)[:5]
            ]
        }
