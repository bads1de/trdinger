...
class InteractionFeatureCalculator:
    ...
    @safe_ml_operation(default_value=None, error_message="相互作用特徴量計算でエラーが発生しました")
    def _calculate_interaction_features_internal(self, df: pd.DataFrame) -> pd.DataFrame:
        """
        内部的な相互作用特徴量計算メソッド
        """
        result_df = df.copy()
    ...
    def _check_required_features(self, df: pd.DataFrame) -> bool:
        ...
        required_features = [
            'Price_Momentum_14', 'Price_Change_5',
            'Volume_Ratio', 'Trend_Strength', 'Breakout_Strength',
            'RSI', 'FR_Normalized',
            'OI_Change_Rate', 'OI_Trend'
        ]
        ...
        if missing_features or not has_atr:
            ...
            if not has_atr:
                logger.warning(f"ATR系の特徴量が見つかりません。利用可能なATR: {[col for col in df.columns if 'ATR' in col]}")
            return False

        return True
    
    def _calculate_volatility_momentum_interactions(self, df: pd.DataFrame) -> pd.DataFrame:
    ...
    def _calculate_volume_trend_interactions(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if 'Volume_Spike' in df.columns and 'Breakout_Strength' in df.columns:
            breakout_strength = df['Breakout_Strength'].replace([np.inf, -np.inf], np.nan)
    ...
    def _calculate_fr_rsi_interactions(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if 'FR_Extreme_High' in df.columns and 'RSI' in df.columns:
            interaction = df['FR_Extreme_High'].astype(float) * (df['RSI'] > 70).astype(float)
        ...
        if 'FR_Extreme_Low' in df.columns and 'RSI' in df.columns:
            interaction = df['FR_Extreme_Low'].astype(float) * (df['RSI'] < 30).astype(float)
