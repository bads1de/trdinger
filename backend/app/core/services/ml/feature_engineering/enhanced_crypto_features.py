...
class EnhancedCryptoFeatures:
    ...
    def _ensure_data_quality(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for col in ['open_interest', 'funding_rate', 'fear_greed_value']:
            result_df[col] = result_df[col].ffill().fillna(
                result_df[col].median() if result_df[col].notna().any() else 0
            )

        return result_df

    def _create_price_features(self, df: pd.DataFrame, periods: Dict[str, int]) -> pd.DataFrame:
        """価格関連特徴量"""
        result_df = df.copy()
        ...
        for name, period in periods.items():
            ...
            result_df[f'price_volatility_{name}'] = df['Close'].rolling(period).std() / df['Close'].rolling(period).mean()
        ...
        for period in [24, 168]:  # 1日、1週間
            result_df[f'price_vs_high_{period}h'] = df['Close'] / df['High'].rolling(period).max()
            result_df[f'price_vs_low_{period}h'] = df['Close'] / df['Low'].rolling(period).min()

        self.feature_groups['price'].extend([
            col for col in result_df.columns if col.startswith(('price_', 'upper_', 'lower_', 'body_'))
        ])

        return result_df

    def _create_volume_features(self, df: pd.DataFrame, periods: Dict[str, int]) -> pd.DataFrame:
        """出来高関連特徴量"""
        result_df = df.copy()
        ...
        for name, period in periods.items():
            ...
            result_df[f'volume_ratio_{name}'] = df['Volume'] / df['Volume'].rolling(period).mean()
    ...
    def _create_open_interest_features(self, df: pd.DataFrame, periods: Dict[str, int]) -> pd.DataFrame:
        """建玉残高関連特徴量"""
        result_df = df.copy()
        ...
        for period in [24, 72, 168]:
            result_df[f'oi_momentum_{period}h'] = df['open_interest'].rolling(period).apply(
    ...
    def _create_funding_rate_features(self, df: pd.DataFrame, periods: Dict[str, int]) -> pd.DataFrame:
        """ファンディングレート関連特徴量"""
        result_df = df.copy()
        ...
        result_df['fr_extreme_positive'] = (df['funding_rate'] > fr_quantiles[0.95]).astype(int)
        result_df['fr_extreme_negative'] = (df['funding_rate'] < fr_quantiles[0.05]).astype(int)
    ...
    def _create_fear_greed_features(self, df: pd.DataFrame, periods: Dict[str, int]) -> pd.DataFrame:
        """Fear & Greed関連特徴量"""
        result_df = df.copy()
    ...
    def _create_technical_features(self, df: pd.DataFrame, periods: Dict[str, int]) -> pd.DataFrame:
        """テクニカル指標"""
        result_df = df.copy()
        ...
        self.feature_groups['technical'].extend([
            col for col in result_df.columns if col.startswith(('rsi_', 'bb_', 'macd'))
        ])

        return result_df

    def _create_composite_features(self, df: pd.DataFrame, periods: Dict[str, int]) -> pd.DataFrame:
        """複合特徴量（相互作用）"""
        result_df = df.copy()

        # 価格 vs OI の関係
        result_df['price_oi_correlation'] = df['Close'].rolling(24).corr(df['open_interest'])

        # 出来高 vs 価格変動の関係
        result_df['volume_price_efficiency'] = (
            df['Volume'] / (abs(df['Close'].pct_change()) + 1e-10)
        )
        ...
        self.feature_groups['composite'].extend([
            col for col in result_df.columns if col.startswith(('price_oi_', 'volume_price_', 'multi_', 'market_'))
        ])
    ...
    def _create_temporal_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        self.feature_groups['temporal'].extend([
            col for col in result_df.columns if col.startswith(('hour', 'day_', 'is_', 'asia_', 'europe_', 'us_', 'month_'))
        ])
