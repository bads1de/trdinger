...
class TemporalFeatureCalculator:
    ...
    def calculate_temporal_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if not isinstance(df.index, pd.DatetimeIndex):
            raise ValueError("DataFrameのインデックスはDatetimeIndexである必要があります")

        return self._calculate_temporal_features_internal(df)

    @safe_ml_operation(default_value=None, error_message="時間的特徴量計算でエラーが発生しました")
    ...
    def _calculate_basic_time_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # 時間（0-23）
        result_df['Hour_of_Day'] = df.index.hour
        
        # 曜日（0=月曜日, 6=日曜日）
        result_df['Day_of_Week'] = df.index.dayofweek
        
        # 週末フラグ（土日）
        result_df['Is_Weekend'] = df.index.dayofweek.isin([5, 6])
        
        # 月曜効果
        result_df['Is_Monday'] = df.index.dayofweek == 0
        
        # 金曜効果
        result_df['Is_Friday'] = df.index.dayofweek == 4
    ...
    def _calculate_cyclical_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # 曜日の周期的エンコーディング（7日周期）
        day_angle = 2 * np.pi * df.index.dayofweek / 7
