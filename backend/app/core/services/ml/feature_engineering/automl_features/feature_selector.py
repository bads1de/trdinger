...
class AdvancedFeatureSelector:
    ...
    def select_features_comprehensive(
        ...
        logger.info(f"包括的特徴量選択を開始: {len(features.columns)}個 → {max_features}個")
    ...
    def _statistical_test_selection(
        ...
        try:
            ...
            if len(valid_features) == 0:
                ...
                return features, {"method": "statistical_test", "selected": 0, "error": "no_valid_data"}
            ...
            step_info = {
                ...
                "mean_f_score": float(valid_scores['f_score'].mean()) if len(valid_scores) > 0 else 0
            }
            ...
            return features, {"method": "statistical_test", "selected": 0, "error": str(e)}

    def _correlation_filter_selection(
        self, features: pd.DataFrame, target: pd.Series, correlation_threshold: float = 0.95
    ) -> Tuple[pd.DataFrame, Dict[str, Any]]:
        """相関フィルタリングによる特徴量選択"""
        try:
            ...
            # 除去対象以外の特徴量を選択
            selected_features = [col for col in features.columns if col not in removed_features]
            result_features = features[selected_features]
            ...
            return features, {"method": "correlation_filter", "selected": 0, "error": str(e)}
    ...
    def _mutual_information_selection(
        ...
        try:
            ...
            if len(valid_features) == 0:
                logger.warning("有効なデータが見つかりません")
                return features, {"method": "mutual_information", "selected": 0, "error": "no_valid_data"}

            # 相互情報量を計算
            with warnings.catch_warnings():
                warnings.simplefilter("ignore")
                mi_scores = mutual_info_regression(valid_features, valid_target, random_state=42)
            ...
            return features, {"method": "mutual_information", "selected": 0, "error": str(e)}
    ...
    def _importance_based_selection(
        ...
        try:
            ...
            if len(valid_features) == 0:
                logger.warning("有効なデータが見つかりません")
                return features, {"method": "importance_based", "selected": 0, "error": "no_valid_data"}
            ...
            selected_features = feature_importance.head(selected_count)['feature'].tolist()
            ...
            return features, {"method": "importance_based", "selected": 0, "error": str(e)}
