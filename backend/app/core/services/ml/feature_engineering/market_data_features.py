...
class MarketDataFeatureCalculator:
    """
    市場データ特徴量計算クラス
    ...
    def calculate_funding_rate_features(
        ...
        try:
            ...
            result_df['FR_MA_24'] = DataValidator.safe_rolling_mean(merged_df[fr_column], window=24)
            result_df['FR_MA_168'] = DataValidator.safe_rolling_mean(merged_df[fr_column], window=168)  # 1週間

            # ファンディングレート変化（安全な計算）
            result_df['FR_Change'] = merged_df[fr_column].diff().fillna(0.0)
            result_df['FR_Change_Rate'] = DataValidator.safe_pct_change(merged_df[fr_column])
            ...
            result_df['FR_Extreme_High'] = (merged_df[fr_column] > fr_quantile_high).astype(int)
            ...
            # ファンディングレートトレンド
            result_df['FR_Trend'] = (
                result_df['FR_MA_24'] - result_df['FR_MA_168']
            )
            
            # ファンディングレートボラティリティ（安全な計算）
            result_df['FR_Volatility'] = DataValidator.safe_rolling_std(merged_df[fr_column], window=24)
    ...
    def calculate_open_interest_features(
        ...
        try:
            ...
            # 建玉残高変化率（安全な計算）
            result_df['OI_Change_Rate'] = DataValidator.safe_pct_change(merged_df[oi_column])
            result_df['OI_Change_Rate_24h'] = DataValidator.safe_pct_change(merged_df[oi_column], periods=24)
            ...
            # 建玉残高移動平均（安全な計算）
            result_df['OI_MA_24'] = DataValidator.safe_rolling_mean(merged_df[oi_column], window=24)
            result_df['OI_MA_168'] = DataValidator.safe_rolling_mean(merged_df[oi_column], window=168)
            
            # 建玉残高トレンド
            result_df['OI_Trend'] = (
                result_df['OI_MA_24'] / result_df['OI_MA_168'] - 1
            )
    ...
    def calculate_composite_features(
        ...
        try:
            ...
            if fr_column is None or oi_column is None:
                logger.warning("ファンディングレートまたは建玉残高カラムが見つかりません")
                return result_df
            ...
            # 市場バランス指標
            result_df['Market_Balance'] = (
                (merged_df[fr_column] > 0).astype(int) * 
                (oi_change > 0).astype(int)
            )
    ...
    def get_feature_names(self) -> list:
        ...
        return [
            ...
            # 建玉残高特徴量
            'OI_Change_Rate', 'OI_Change_Rate_24h', 'OI_Surge',
            'Volatility_Adjusted_OI', 'OI_Trend', 'OI_Price_Correlation',
            'OI_Normalized',
            
            # 複合特徴量
            'FR_OI_Ratio', 'Market_Heat_Index', 'Market_Stress',
            'Market_Balance'
        ]
