...
class LightGBMTrainer(BaseMLTrainer):
    ...
    def predict(self, features_df: pd.DataFrame) -> np.ndarray:
        ...
        try:
            ...
            # 予測実行
            predictions = self.model.predict(features_scaled, num_iteration=self.model.best_iteration)
...
class RandomForestTrainer(BaseMLTrainer):
    """
    RandomForestを使用したML学習クラス（互換性のため）
    
    既存のMLIndicatorServiceとの互換性を保つために提供します。
    """
    ...
    def predict(self, features_df: pd.DataFrame) -> np.ndarray:
        ...
        try:
            ...
            if predictions.shape[1] == 3:
                ...
                prob_up = predictions[:, 1]
                prob_range = np.zeros_like(prob_down)
                return np.column_stack([prob_down, prob_range, prob_up])
            else:
                # デフォルト値
                return np.full((len(features_df), 3), 1/3)
    ...
    def _train_model_impl(
        ...
        **training_params
        ...
        try:
            ...
            logger.info(f"RandomForestモデル学習完了: 精度={accuracy:.4f}")
            return result
            
        except Exception as e:
            logger.error(f"RandomForestモデル学習エラー: {e}")
            raise MLModelError(f"RandomForestモデル学習に失敗しました: {e}")
