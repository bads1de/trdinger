...
class ExternalMarketService:
    ...
    def fetch_external_market_data(
        ...
        try:
            ...
            logger.info(f"外部市場データを取得中: symbols={target_symbols}, period={period}")
            ...
            for symbol in target_symbols:
                try:
                    ...
                    if hist_data.empty:
                        logger.warning(f"シンボル {symbol} のデータが取得できませんでした")
                        continue

                    # データ変換
                    symbol_data = self._convert_yfinance_data_to_db_format(symbol, hist_data)
                    all_data.extend(symbol_data)

                    logger.info(f"シンボル {symbol} のデータを {len(symbol_data)} 件取得しました")
        ...
        for index, row in yfinance_data.iterrows():
            try:
                ...
                if all(x <= 0 for x in [open_price, high_price, low_price, close_price]):
                    logger.warning(f"シンボル {symbol} の {data_timestamp} のデータが無効です（全て0以下）")
                    continue
                ...
                logger.warning(f"データ変換エラー、スキップします: {index}, エラー: {e}")
                continue
    ...
    async def fetch_and_save_external_market_data(
        ...
        try:
            ...
            # データベースに保存
            if repository:
                inserted_count = repository.insert_external_market_data(external_market_data)
            else:
