...
class OperandGroupingSystem:
    ...
    def _initialize_compatibility_matrix(self) -> Dict[Tuple[OperandGroup, OperandGroup], float]:
        """グループ間の互換性マトリックスを初期化
    ...
    def _classify_by_pattern(self, operand: str) -> OperandGroup:
        ...
        # 0-100%オシレーターのパターン
        if any(pattern in operand_upper for pattern in [
            "RSI", "STOCH", "ADX", "WILLR", "MFI", "ULTOSC"
        ]):
        ...
        # ゼロ中心のパターン
        if any(pattern in operand_upper for pattern in [
            "MACD", "TRIX", "PPO", "MOM", "BOP", "APO", "EMV", "ROCP", "ROCR", "OBV"
        ]):
            return OperandGroup.ZERO_CENTERED
        
        # 特殊スケールのパターン
        if any(pattern in operand_upper for pattern in [
            "OPENINTEREST", "FUNDINGRATE", "VOLUME"
        ]):
    ...
    def get_operands_by_group(self, group: OperandGroup) -> List[str]:
        ...
        """
        return [
            operand for operand, operand_group in self._group_mappings.items()
    ...
    def validate_condition(self, left_operand: str, right_operand: str) -> Tuple[bool, str]:
        """条件の妥当性を検証
