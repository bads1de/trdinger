...
class GeneEncoder:
    ...
    def _get_indicator_ids(self) -> Dict[str, int]:
        ...
        try:
            ...
            # フォールバック: 基本的な指標のみ
            return {
                "": 0, "SMA": 1, "EMA": 2, "RSI": 3, "MACD": 4, "BB": 5,
                "STOCH": 6, "CCI": 7, "WILLIAMS": 8, "ADX": 9, "ATR": 10
            }
    ...
    def encode_strategy_gene_to_list(self, strategy_gene) -> List[float]:
        ...
        try:
            ...
            for i in range(max_indicators):
                if i < len(strategy_gene.indicators) and strategy_gene.indicators[i].enabled:
                    indicator = strategy_gene.indicators[i]
                    indicator_id = self.indicator_ids.get(indicator.type, 0)
                    # パラメータを正規化（期間の場合は1-200を0-1に変換）
                    param_val = self._normalize_parameter(indicator.parameters.get("period", 20))
                else:
                    indicator_id = 0  # 未使用
                    param_val = 0.0
        ...
        try:
            ...
            for i in range(max_indicators):
                ...
                if idx + 1 < len(encoded):
                    ...
                    if indicator_type:
                        # 指標タイプに応じたパラメータ生成
                        parameters = self._generate_indicator_parameters(indicator_type, param_val)
            ...
            if indicators:
                ...
                # 指標タイプに応じた条件を生成
                entry_conditions, exit_conditions = self._generate_indicator_specific_conditions(
    ...
    def _normalize_parameter(self, value: float, min_val: float = 1, max_val: float = 200) -> float:
        """パラメータを0-1の範囲に正規化"""
        try:
    ...
    def _denormalize_parameter(self, normalized_val: float, min_val: float = 1, max_val: float = 200) -> int:
        """正規化されたパラメータを元の範囲に戻す"""
        try:
    ...
    def _generate_indicator_parameters(self, indicator_type: str, param_val: float) -> Dict:
        """指標タイプに応じたパラメータを生成"""
        try:
            # 基本的な期間パラメータ
            if indicator_type in ["SMA", "EMA", "RSI", "ATR", "CCI", "WILLIAMS", "MFI", "MOMENTUM", "ROC"]:
                period = self._denormalize_parameter(param_val, min_val=5, max_val=50)
                return {"period": period}
            
            # 複数パラメータを持つ指標
            elif indicator_type == "MACD":
                fast_period = self._denormalize_parameter(param_val, min_val=5, max_val=20)
                slow_period = fast_period + self._denormalize_parameter(param_val, min_val=5, max_val=15)
                signal_period = self._denormalize_parameter(param_val, min_val=5, max_val=15)
    ...
    def _generate_indicator_specific_conditions(self, indicator, indicator_name):
        ...
        try:
            ...
            if indicator.type == "RSI":
                ...
                # MACD条件
                entry_conditions = [
                    Condition(left_operand="MACD_line", operator=">", right_operand="MACD_signal")
                ]
                exit_conditions = [
                    Condition(left_operand="MACD_line", operator="<", right_operand="MACD_signal")
                ]
                ...
                # デフォルト条件
                entry_conditions = [
                    Condition(left_operand=indicator_name, operator=">", right_operand=50)
                ]
                exit_conditions = [
                    Condition(left_operand=indicator_name, operator="<", right_operand=50)
                ]
