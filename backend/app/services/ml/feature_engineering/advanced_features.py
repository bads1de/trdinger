...
class AdvancedFeatureEngineer:
    ...
    def _add_advanced_technical_indicators(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            data['SHOOTING_STAR'] = talib.CDLSHOOTINGSTAR(data['Open'], high, low, close)
    ...
    def _add_statistical_features(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        for window in windows:
            ...
            # 範囲統計
            data[f'Close_range_{window}'] = data['High'].rolling(window).max() - data['Low'].rolling(window).min()
            data[f'Close_iqr_{window}'] = data[f'Close_q75_{window}'] - data[f'Close_q25_{window}']
    ...
    def _add_volatility_features(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        for window in [5, 10, 20, 50]:
            data[f'Realized_Vol_{window}'] = data['Returns'].rolling(window).std() * np.sqrt(24)  # 日次換算
            data[f'Vol_of_Vol_{window}'] = data[f'Realized_Vol_{window}'].rolling(window).std()
        
        # Parkinson推定量（高値・安値ベースのボラティリティ）
        for window in [10, 20]:
            hl_ratio = np.log(data['High'] / data['Low'])
            data[f'Parkinson_Vol_{window}'] = hl_ratio.rolling(window).var() * (1 / (4 * np.log(2)))
        
        # ボラティリティレジーム
        vol_20 = data['Returns'].rolling(20).std()
        data['Vol_Regime'] = pd.cut(vol_20, bins=3, labels=[0, 1, 2])  # 低・中・高ボラティリティ
        
        return data

    def _add_funding_rate_features(self, data: pd.DataFrame, fr_data: pd.DataFrame) -> pd.DataFrame:
        """ファンディングレート特徴量を追加"""
        ...
        if 'funding_rate' in fr_data.columns:
            ...
            for window in [3, 7, 14]:  # 3回、7回、14回分（24h, 56h, 112h）
                data[f'FR_mean_{window}'] = fr_data['funding_rate'].rolling(window).mean()
            ...
            # 極端なファンディングレート
            data['FR_extreme_positive'] = (fr_data['funding_rate'] > fr_data['funding_rate'].quantile(0.95)).astype(int)
            data['FR_extreme_negative'] = (fr_data['funding_rate'] < fr_data['funding_rate'].quantile(0.05)).astype(int)
        
        return data

    def _add_open_interest_features(self, data: pd.DataFrame, oi_data: pd.DataFrame) -> pd.DataFrame:
        """建玉残高特徴量を追加"""
        ...
        if 'open_interest' in oi_data.columns:
            # 建玉残高の変化率
            for period in [1, 6, 24]:
                data[f'OI_pct_change_{period}'] = oi_data['open_interest'].pct_change(period)
            
            # 建玉残高の移動平均
            for window in [6, 24, 168]:  # 6h, 24h, 168h(1週間)
                data[f'OI_ma_{window}'] = oi_data['open_interest'].rolling(window).mean()
                data[f'OI_deviation_{window}'] = (oi_data['open_interest'] - data[f'OI_ma_{window}']) / data[f'OI_ma_{window}']
            
            # 建玉残高と価格の関係
            data['OI_Price_Correlation'] = oi_data['open_interest'].rolling(24).corr(data['Close'])
    ...
    def _add_seasonal_features(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        data['Is_European_Hours'] = ((data['Hour'] >= 8) & (data['Hour'] < 16)).astype(int)
        data['Is_American_Hours'] = ((data['Hour'] >= 16) & (data['Hour'] < 24)).astype(int)
    ...
    def clean_features(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        for col in numeric_columns:
            if col not in ['Hour', 'DayOfWeek', 'DayOfMonth', 'Month', 'Is_Weekend', 'Is_Asian_Hours', 'Is_European_Hours', 'Is_American_Hours']:
