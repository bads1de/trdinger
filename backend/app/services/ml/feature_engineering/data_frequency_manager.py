...
class DataFrequencyManager:
    ...
    def align_data_frequencies(
        ...
        try:
            ...
            if funding_rate_data is not None and not funding_rate_data.empty:
                aligned_fr_data = self._resample_funding_rate(funding_rate_data, ohlcv_timeframe)

            # 建玉残高データの再サンプリング
            aligned_oi_data = None
            if open_interest_data is not None and not open_interest_data.empty:
                aligned_oi_data = self._resample_open_interest(open_interest_data, ohlcv_timeframe)
    ...
    def _resample_funding_rate(self, fr_data: pd.DataFrame, target_timeframe: str) -> pd.DataFrame:
        """
        ...
        try:
            ...
            logger.info(f"ファンディングレート再サンプリング完了: {len(fr_data)} → {len(resampled)}行")
            return resampled
    ...
    def _resample_open_interest(self, oi_data: pd.DataFrame, target_timeframe: str) -> pd.DataFrame:
        """
        建玉残高データを再サンプリング
        ...
        try:
            ...
            logger.info(f"建玉残高再サンプリング完了: {len(oi_data)} → {len(resampled)}行")
            return resampled
