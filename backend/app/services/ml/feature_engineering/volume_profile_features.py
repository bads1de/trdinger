...
def _numba_detect_volume_nodes_signed(
    ...
    for i in range(window, n):
        ...
        lvn_th, hvn_th = (
            sorted_vol[int(0.25 * (num_bins - 1))],
            sorted_vol[int(0.75 * (num_bins - 1))],
        )
        curr_p = close_arr[i]
        ...
        for b in range(num_bins):
            ...
            if bin_vol[b] >= hvn_th:
                d = abs(curr_p - p)
                if d < min_h_d:
                    min_h_d, best_hvn = d, p
            if bin_vol[b] <= lvn_th:
                d = abs(curr_p - p)
                if d < min_l_d:
                    min_l_d, best_lvn = d, p
        if best_hvn != -1.0 and curr_p != 0:
            hvn_dist[i] = (curr_p - best_hvn) / curr_p
        if best_lvn != -1.0 and curr_p != 0:
            lvn_dist[i] = (curr_p - best_lvn) / curr_p
...
class VolumeProfileFeatureCalculator:
    ...
    def calculate_features(
        ...
        for period in lookback_periods:
            poc, vah, val = _numba_rolling_volume_profile(
        ...
        skew, kurt = _numba_vp_skewness_kurtosis(
