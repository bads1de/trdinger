...
class TrainingMetadata:
    """
    学習プロセス全体のメタデータを管理するdataclass
    
    学習の設定、データ情報、結果を包括的に管理します。
    """
    ...
    def log_summary(self) -> None:
        """学習メタデータのサマリーをログ出力"""
        logger.info(
            f"学習メタデータ: サンプル数={self.total_samples}, "
            f"特徴量数={self.feature_count}, モデル={self.model_type}"
        )
        
        if self.label_distribution:
            logger.info(f"ラベル分布: {self.label_distribution}")
        
        if self.model_metadata:
            self.model_metadata.log_summary()