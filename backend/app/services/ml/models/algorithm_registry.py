...
class AlgorithmRegistry:
    ...
    def get_algorithms_by_type(self, algorithm_type: AlgorithmType) -> List[str]:
        ...
        """
        return [
            name for name, info in self._algorithms.items()
            if info['type'] == algorithm_type
        ]

    def get_algorithms_by_capability(self, capability: AlgorithmCapability) -> List[str]:
        """
        指定された機能を持つアルゴリズムを取得
    ...
    def create_algorithm_instance(self, algorithm_name: str, automl_config: Optional[Dict[str, Any]] = None):
        """
        ...
        if algorithm_name not in self._algorithms:
            ...
            raise ValueError(f"未知のアルゴリズム: {algorithm_name}. 利用可能: {available}")
        ...
        try:
            ...
            raise ImportError(f"アルゴリズム {algorithm_name} のクラス {class_name} が見つかりません: {e}")
...
if __name__ == "__main__":
    ...
    print(f"確率予測対応: {len(summary['by_capability'].get('probability_prediction', []))}個")
    print(f"特徴量重要度対応: {len(summary['by_capability'].get('feature_importance', []))}個")
