...
アンサンブル学習で使用するExtraTreesモデルのラッパークラスを提供します。
scikit-learnのExtraTreesClassifierを使用してアンサンブル専用に最適化されたモデルです。
"""
...
class ExtraTreesModel:
    """
    アンサンブル内で使用するExtraTreesモデルラッパー

    scikit-learnのExtraTreesClassifierを使用してアンサンブル専用に最適化されたモデル
    """
    ...
    def _train_model_impl(
        ...
        **training_params,
        ...
        """
        try:
            logger.info("🌲 ExtraTreesモデルの学習を開始")

            # 特徴量カラムを保存
            self.feature_columns = list(X_train.columns)

            # モデル初期化
            self.model = ExtraTreesClassifier(**self.default_params)
            ...
            logger.info(
                f"✅ ExtraTrees学習完了 - テスト精度: {test_metrics.get('accuracy', 0.0):.4f}"
            )
            return results
