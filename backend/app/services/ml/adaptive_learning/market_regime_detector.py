...
class MarketRegimeDetector:
    ...
    def _calculate_regime_indicators(self, data: pd.DataFrame) -> Dict[str, float]:
        ...
        try:
            ...
            indicators['trend_strength'] = abs(returns.mean()) / returns.std() if returns.std() > 0 else 0
            ...
            indicators['volume_ratio'] = volume.iloc[-1] / volume_ma.iloc[-1] if volume_ma.iloc[-1] > 0 else 1
            
            # 6. ブレイクアウト指標
            bb_upper, bb_lower = self._calculate_bollinger_bands(close, period=20, std_dev=2)
            ...
            # 7. 反転指標
            indicators['price_deviation'] = (close.iloc[-1] - close.rolling(20).mean().iloc[-1]) / close.rolling(20).std().iloc[-1]
    ...
    def _classify_regime(self, indicators: Dict[str, float]) -> Tuple[MarketRegime, float]:
        ...
        try:
            ...
            scores[MarketRegime.RANGING] = (
                ...
                max(0, self.volatility_threshold - indicators.get('volatility', 0)) * 1
            )
            
            # 高ボラティリティ
            scores[MarketRegime.VOLATILE] = (
                max(0, indicators.get('volatility', 0) - self.volatility_threshold * 2) * 3 +
                max(0, indicators.get('volatility_percentile', 0.5) - 0.7) * 2
            )
            
            # 低ボラティリティ
            scores[MarketRegime.CALM] = (
                max(0, self.volatility_threshold * 0.5 - indicators.get('volatility', 0)) * 3 +
    ...
    def _calculate_bollinger_bands(self, close: pd.Series, period: int = 20, std_dev: float = 2) -> Tuple[float, float]:
        """ボリンジャーバンドを計算"""
        try:
            ...
            return close.iloc[-1], close.iloc[-1]
    ...
    def should_retrain_model(self, stability_threshold: float = 0.7) -> bool:
        ...
        # 再学習条件
        should_retrain = (
            stability < stability_threshold or  # 不安定
            regime_changes >= 3  # 頻繁な変化
        )
        
        if should_retrain:
            logger.info(f"モデル再学習推奨: 安定性={stability:.2f}, 変化数={regime_changes}")
