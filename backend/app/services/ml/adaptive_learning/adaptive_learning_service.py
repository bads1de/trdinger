...
class AdaptiveLearningService:
    ...
    @safe_ml_operation(
        default_return=None, context="適応的学習処理でエラーが発生しました"
    )
    ...
    def _detect_regime_change(self, regime_result: RegimeDetectionResult) -> bool:
        ...
        regime_changed = (
            self.current_regime != regime_result.regime
            and regime_result.confidence >= self.config.regime_change_sensitivity
        )

        if regime_changed:
            logger.info(
                f"レジーム変化検出: {self.current_regime.value} → {regime_result.regime.value}"
            )
            self.current_regime = regime_result.regime
    ...
    def _execute_adaptation_action(
        self,
        action: str,
        market_data: pd.DataFrame,
        regime_result: RegimeDetectionResult,
    ...
    def _retrain_model(self, market_data: pd.DataFrame, regime: MarketRegime) -> bool:
        ...
        try:
            ...
            if len(market_data) < self.config.min_data_points:
                logger.warning(
                    f"データ不足: {len(market_data)} < {self.config.min_data_points}"
                )
                return False
    ...
    def _adjust_model_parameters(self, regime: MarketRegime) -> bool:
        ...
        try:
            ...
            # レジーム特化のパラメータ調整
            self._get_regime_parameter_adjustments(regime)
    ...
    def _get_regime_specific_params(self, regime: MarketRegime) -> Dict[str, Any]:
        ...
        if regime == MarketRegime.VOLATILE:
            ...
            base_params.update(
                {"scale_features": True, "scaling_method": "standard"}  # 標準化を使用
            )
