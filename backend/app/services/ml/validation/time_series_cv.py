...
class TimeSeriesCrossValidator:
        ...
        **fit_params
        ...
        for fold, (train_idx, test_idx) in enumerate(splitter.split(X), 1):
            ...
            try:
                ...
                logger.info(f"フォールド {fold} 完了: 精度={fold_scores.get('accuracy', 0.0):.4f}")
    ...
    def _validate_data(self, X: pd.DataFrame, y: pd.Series):
        ...
        if len(X) < self.config.n_splits * self.config.min_train_size:
            raise ValueError(f"データが不足しています: {len(X)} < {self.config.n_splits * self.config.min_train_size}")
            ...
            if 'roc_auc' in scoring and y_proba is not None:
                try:
                    if y_proba.ndim == 2 and y_proba.shape[1] > 2:
                        ...
                        # 二値分類の場合
                        scores['roc_auc'] = roc_auc_score(y_true, y_proba[:, 1] if y_proba.ndim == 2 else y_proba)
                except Exception as e:
    ...
    def _aggregate_results(self, scores: Dict[str, List[float]], fold_results: List[Dict]) -> Dict[str, Any]:
        """結果を集計"""
        aggregated = {
