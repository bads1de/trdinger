...
class EnsembleTrainer(BaseMLTrainer):
    ...
    def __init__(
        self,
        ensemble_config: Dict[str, SerializableValue],
    ):
        """
        アンサンブルトレーナーを初期化します。

        Args:
            ensemble_config (Dict[str, SerializableValue]): アンサンブルの構成設定。
    ...
    def _extract_optimized_parameters(
        self, training_params: Dict[str, SerializableValue]
    ) -> Dict[str, Dict[str, SerializableValue]]:
        """
        ハイパーパラメータ最適化（Optuna等）によって得られた平坦なパラメータ辞書を、
        各ベースモデルおよびメタモデル（スタッキング）用の構造化された辞書に分離します。

        Args:
            training_params (Dict[str, SerializableValue]): 最適化された全パラメータを含む辞書。
                プレフィックス（"lgb_", "xgb_", "stacking_"等）によって分類されます。

        Returns:
            Dict[str, Dict[str, SerializableValue]]: 構造化されたパラメータ辞書。
                - "base_models": 各アルゴリズム（lightgbm, xgboost等）のパラメータ。
                - "stacking": メタモデルやスタッキング自体の設定。
        """
        optimized_params: Dict[str, SerializableValue] = {
    ...
    def _build_base_model_params(
        ...
        base_model_params: Dict[str, Dict[str, SerializableValue]] = {
            "lightgbm": {
