...
class EnsembleConfig(TypedDict, total=False):
    """アンサンブル設定の型定義"""
...
class EnsembleTrainer(BaseMLTrainer):
    ...
    def __init__(
        ...
        Args:
            ensemble_config (EnsembleConfig): アンサンブルの構成設定。
        ...
        self.ensemble_model: StackingEnsemble | None = None
        self.meta_labeling_service: MetaLabelingService | None = (
        ...
        models: list[str] = ensemble_config.get("models") or []
    ...
    def load_model(self, model_path: str) -> bool:  # type: ignore[override]
        ...
        try:
            ...
            self.ensemble_model = cast(StackingEnsemble | None, self._model)
            ...
            if meta_model_path:
                try:
                    ...
                    if not meta_data:
                        ...
                        meta_filename = os.path.basename(str(meta_model_path))
