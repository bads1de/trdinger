...
class BacktestDataProvider:
    ...
    def _extract_worker_data(
        ...
        if worker_key == expected_key:
            return (
                worker_data if isinstance(worker_data, pd.DataFrame) else None
            )
        if not BacktestDataProvider._is_compatible_worker_key(
        ...
        try:
            ...
            expected_range = BacktestDataProvider._parse_key_date_range(
                expected_key
            )
            ...
            if len(worker_index) > 0:
                ...
                expected_end = align_timestamp_to_index(
                    expected_end, worker_index
                )
        ...
        sliced = worker_data.loc[
            (worker_data.index >= expected_start)
            & (worker_data.index <= expected_end)
        ]
    ...
    def _is_compatible_worker_key(
        ...
        if not isinstance(worker_key, tuple) or len(worker_key) != len(
            expected_key
        ):
            return False
        ...
        with self._lock:
            if cache_key in self._data_cache:
                ...
                if hasattr(cached_data, "empty") and not cached_data.empty:
                    self._cache_hits += 1
                    logger.debug(
                        f"OHLCVデータ: キャッシュヒット (key={cache_key})"
                    )
                    return cached_data
    ...
    def prefetch_data(
        ...
        def _prefetch_task():
            try:
                data_service = getattr(
                    self.backtest_service, "data_service", None
                )
                if data_service is None:
                    return
                ...
                if (
                    isinstance(ohlcv_data, pd.DataFrame)
                    and not ohlcv_data.empty
                ):
                    with self._lock:
