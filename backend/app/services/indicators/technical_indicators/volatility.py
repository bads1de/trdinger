...
class VolatilityIndicators:
    ...
    @staticmethod
    @handle_pandas_ta_errors
    def hwc(close: pd.Series, **kwargs) -> Tuple[pd.Series, pd.Series, pd.Series]:
        ...
        if result is None or (hasattr(result, "isna") and result.isna().all().all()):
            ...
            try:
                ...
                period = max(
                    20, len(close) // 8
                )  # adaptive period based on data length
        ...
        cols = list(result.columns)
        upper = result[
            next((c for c in cols if "HWU" in c or "upper" in c.lower()), cols[0])
        ]
        middle = result[
            next(
        ...
        lower = result[
            next((c for c in cols if "HWL" in c or "lower" in c.lower()), cols[-1])
        ]
    ...
    @staticmethod
    @handle_pandas_ta_errors
    def variance(data: pd.Series, period: int = 14) -> pd.Series:
    ...
    @staticmethod
    @handle_pandas_ta_errors
    def coefficient_of_variation(data: pd.Series, period: int = 14) -> pd.Series:
    ...
    def vortex(
        ...
        try:
            ...
            if len(vi_plus_series) > length:
                vi_plus_series = (
                    vi_plus_series.rolling(window=min(length, 5))
                    .mean()
                    .fillna(vi_plus_series)
                )
                vi_minus_series = (
                    vi_minus_series.rolling(window=min(length, 5))
                    .mean()
                    .fillna(vi_minus_series)
                )
