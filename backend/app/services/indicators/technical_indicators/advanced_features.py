...
class AdvancedFeatures:
    ...
    def squeeze_probability(
        ...
        prob = pd.Series(neg_fr_factor, index=close.index) * delta_oi_factor * price_location
    ...
    def leverage_ratio(
        ...
        if market_cap is None or market_cap.empty:
             return pd.Series(np.full(len(open_interest), np.nan), index=open_interest.index)
