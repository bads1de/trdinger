...
class StrategyClassFactory:
    ...
    def _is_auto_strategy(self, strategy_config: Dict[str, Any]) -> bool:
        """オートストラテジーかどうかを判定"""
        return (
            "strategy_gene" in strategy_config or
            ("parameters" in strategy_config and "strategy_gene" in strategy_config["parameters"])
        )
    
    def _create_auto_strategy_class(self, strategy_config: Dict[str, Any]) -> Type[Strategy]:
        """
        オートストラテジーのクラスを生成
        ...
        try:
            ...
            raise StrategyClassCreationError(f"オートストラテジーモジュールのインポートに失敗しました: {e}")
        ...
        try:
            ...
            logger.debug(f"オートストラテジークラスを生成しました: {strategy_class.__name__}")
    ...
    def get_strategy_parameters(self, strategy_config: Dict[str, Any]) -> Dict[str, Any]:
        """
        戦略設定からパラメータを抽出
