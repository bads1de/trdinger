...
class BacktestConfigValidator:
    ...
    def _validate_field_values(self, config: Dict[str, Any]) -> List[str]:
        ...
        # 戦略名の検証
        if not isinstance(config["strategy_name"], str) or not config["strategy_name"].strip():
        ...
        if config["timeframe"] not in self.VALID_TIMEFRAMES:
            errors.append(f"timeframeは {self.VALID_TIMEFRAMES} のいずれかである必要があります")
