...
class EventDrivenLabelGenerator:
    """イベントドリブン（トリプルバリア）ラベル生成器"""

    REGIME_FACTORS: Dict[Any, Dict[str, float]] = {
    ...
    def generate_hrhp_lrlp_labels(
        ...
        profile_overrides: Optional[Dict[str, Dict[str, float]]] = None,
    ) -> Tuple[pd.DataFrame, Dict[str, Any]]:
        if market_data is None or market_data.empty or len(market_data) < 3:
            logger.warning("市場データが不足しているためイベントラベル生成をスキップ")
    ...
    def _distribution_summary(self, labels: pd.Series) -> Dict[str, float]:
        total = len(labels)
        if total == 0:
