...
class IndexAlignmentManager:
    ...
    def align_data(
        self, features: pd.DataFrame, labels: pd.Series, method: str = "intersection"
        ...
        """
        logger.info(
            f"インデックス整合開始: 特徴量{len(features)}行, ラベル{len(labels)}行"
        )
    ...
    def preserve_index_during_processing(
        self, data: pd.DataFrame, processing_func: callable, *args, **kwargs
    ) -> pd.DataFrame:
        """
        処理中にインデックスを保持
    ...
    def get_alignment_summary(self) -> Dict[str, Any]:
        ...
        summary = {
            ...
            "average_alignment_ratio": np.mean(
                [h["alignment_ratio"] for h in self.alignment_history]
            ),
            "min_alignment_ratio": min(
                [h["alignment_ratio"] for h in self.alignment_history]
            ),
            "max_alignment_ratio": max(
                [h["alignment_ratio"] for h in self.alignment_history]
            ),
        }
