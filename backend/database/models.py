...
class StrategyShowcase(Base):
    ...
    # 戦略カテゴリ（trend_following, mean_reversion, breakout, range_trading）
    ...
    # 推奨時間軸
    recommended_timeframe = Column(String(10), nullable=True)
    ...
    __table_args__ = (
        ...
        Index("idx_showcase_risk_level", "risk_level"),
        Index("idx_showcase_return", "expected_return"),
        Index("idx_showcase_sharpe", "sharpe_ratio"),
        Index("idx_showcase_drawdown", "max_drawdown"),
        Index("idx_showcase_active", "is_active"),
    )

    def __repr__(self):
        return (
            f"<StrategyShowcase(name='{self.name}', "
            f"category='{self.category}', "
            f"return={self.expected_return:.2f if self.expected_return else 'None'}%)>"
        )
