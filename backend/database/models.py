...
class UserStrategy(Base):
    ...
    ストラテジービルダーで作成されたユーザー定義戦略を保存します。
    戦略設定はStrategyGene形式でJSONB形式で保存されます。
    """
    ...
    # インデックス定義
    __table_args__ = (
        Index("idx_user_strategies_name", "name"),
        Index("idx_user_strategies_active", "is_active"),
        Index("idx_user_strategies_created", "created_at"),
    )

    def __repr__(self):
        return (
            f"<UserStrategy(id={self.id}, "
            f"name='{self.name}', "
