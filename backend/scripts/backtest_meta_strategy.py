...
def run_backtest():
    ...
    logger.info("\n=== Comparison ===")
    logger.info(
        f"Return: {stats_base['Return [%]']:.2f}% -> {stats_meta['Return [%]']:.2f}%"
    )
    logger.info(
        f"Sharpe Ratio: {stats_base['Sharpe Ratio']:.4f} -> {stats_meta['Sharpe Ratio']:.4f}"
    )
    logger.info(
        f"Win Rate: {stats_base['Win Rate [%]']:.2f}% -> {stats_meta['Win Rate [%]']:.2f}%"
    )
    logger.info(f"Trades: {stats_base['# Trades']} -> {stats_meta['# Trades']}")
    logger.info(
        f"Max Drawdown: {stats_base['Max. Drawdown [%]']:.2f}% -> {stats_meta['Max. Drawdown [%]']:.2f}%"
    )
