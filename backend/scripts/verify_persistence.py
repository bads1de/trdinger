...
def verify_ga_experiments():
    ...
    try:
        with get_db_session() as db:
            ...
            # 統計情報
            stats = exp_repo.get_experiment_statistics()
            print("\n統計情報:")
        ...
        logger.error(f"GA実験データの確認中にエラーが発生しました: {e}")
        import traceback
...
def verify_generated_strategies():
    ...
    try:
        with get_db_session() as db:
            ...
            # 最高戦略を取得
            best_strategies = strategy_repo.get_best_strategies(limit=5)
            ...
            for exp in experiments:
                ...
                if strategies:
                    ...
                    for gen in range(1, exp.total_generations + 1):
                        gen_strategies = strategy_repo.get_strategies_by_generation(
                            exp.id, gen
                        )
                        if gen_strategies:
                            gen_stats = strategy_repo.get_generation_statistics(
                                exp.id, gen
                            )
                            print(
        ...
        logger.error(f"生成戦略データの確認中にエラーが発生しました: {e}")
        import traceback
    ...
    try:
        with get_db_session() as db:
            ...
            for exp in experiments:
                ...
                # フィットネススコアの整合性
                if strategies:
                    max_fitness_in_strategies = [
                        s.fitness_score
                        for s in strategies
                        if s.fitness_score is not None
                    ]
                    if max_fitness_in_strategies:
                        max_fitness = max(max_fitness_in_strategies)
                        if (
                            exp.best_fitness
                            and abs(exp.best_fitness - max_fitness) > 0.0001
                        ):
                            print(
        ...
        logger.error(f"データ整合性の確認中にエラーが発生しました: {e}")
        import traceback
