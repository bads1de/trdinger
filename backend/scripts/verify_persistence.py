                ...
                if strategies:
                    ...
                    for gen in range(1, exp.total_generations + 1):
                        ...
                        if gen_strategies:
                            ...
                            print(f"    世代{gen}: {len(gen_strategies)} 戦略, "
...
def verify_data_integrity():
    ...
    try:
        ...
        for exp in experiments:
            ...
            # フィットネススコアの整合性
            if strategies:
                max_fitness = max(s.fitness_score for s in strategies if s.fitness_score is not None)
                if exp.best_fitness and abs(exp.best_fitness - max_fitness) > 0.0001:
                    print(f"  ⚠️ 警告: 実験の最高フィットネス({exp.best_fitness})と"
