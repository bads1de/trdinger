...
class ComprehensiveTestReport:
    ...
    results_by_category: Dict[TestResultCategory, List[IndicatorTestResult]] = field(
    ...
    def get_summary_stats(self) -> Dict[str, Any]:
        ...
        total_time = sum(
            r.execution_time
            for category_results in self.results_by_category.values()
            for r in category_results
        )

        success_count = len(
            self.results_by_category.get(TestResultCategory.SUCCESS, [])
        )
        stats["全体成功率"] = (
            f"{(success_count / self.total_indicators * 100):.1f}%"
            if self.total_indicators > 0
            else "0%"
        )
        stats["総実行時間（秒）"] = round(total_time, 2)
        stats["平均実行時間（秒）"] = (
            f"{(total_time / self.total_indicators):.4f}"
            if self.total_indicators > 0
            else "0"
        )
...
def create_test_data():
    ...
    np.random.seed(42)
    dates = pd.date_range(
        start="2024-01-01", periods=2000, freq="1h"
    )  # More data points, use 'h' instead of deprecated 'H'
    logger.info(f"Creating test data with {len(dates)} samples")

    base_price = 50000
    price_changes = np.random.normal(
        0, 0.05, 2000
    )  # Increase volatility for better indicator calculation
    ...
    high_prices = [
        price * (1 + abs(np.random.normal(0, 0.005))) for price in close_prices
    ]
    low_prices = [
        price * (1 - abs(np.random.normal(0, 0.005))) for price in close_prices
    ]
    open_prices = [base_price * (1 + np.random.normal(0, 0.002))] + [
        close_prices[i - 1] * (1 + np.random.normal(0, 0.002))
    ...
    # MAEインジケーター用の予測値生成
    predicted_prices = [
        price * (1 + np.random.normal(0, 0.01)) for price in close_prices
    ]
    logger.debug(
        f"Generated all price data: close={len(close_prices)}, high={len(high_prices)}, low={len(low_prices)}, open={len(open_prices)}"
    )
...
def create_regime_specific_test_data(n_samples: int = 100) -> Dict[str, pd.DataFrame]:
    ...
    high_vol_df = pd.DataFrame(
        high_vol_data,
        columns=["open", "high", "low", "close", "volume"],
        index=high_vol_dates,
    )
    for col in ["open", "high", "low", "close"]:
        high_vol_df[col] = base_price + high_vol_df[col] * 100

    return {"trend": trend_df, "range": range_df, "high_volatility": high_vol_df}
...
def get_all_registered_indicators():
    ...
    try:
        ...
        logger.info(
            f"全{len(indicators)}個のインジケーターを取得しました（STC, THERMO, AOBV, HWC, RVGI, PPO, PVO, KVO除外）"
        )
        return indicators
...
def test_indicator_calculation(
    df: DataFrame, indicator_name: str
) -> Tuple[TestResultCategory, Optional[np.ndarray], Optional[str]]:
    """インジケーター計算テスト"""
    ...
    try:
        ...
        try:
            if hasattr(config, "parameters") and config.parameters:
                ...
                for param_name, param_config in config.parameters.items():
                    ...
                    logger.debug(
                        f"{indicator_name}: param {param_name} = {param_config.default_value}"
                    )
                ...
                for param_name, param_range in ranges.items():
                    ...
                    logger.debug(
                        f"{indicator_name}: range param {param_name} = {default_value}"
                    )
        ...
        logger.debug(
            f"{indicator_name}: Calculation finished, result type: {type(result)}"
        )
        ...
        if isinstance(result, np.ndarray):
            ...
            logger.debug(
                f"{indicator_name}: Array result - valid: {valid_count}/{total_count}"
            )
            ...
            if valid_count == 0:
                ...
                logger.info(
                    f"{indicator_name}: Partial NaN - valid: {valid_count}/{total_count}"
                )
                return (
                    TestResultCategory.NAN_VALUES,
                    result,
                    f"{valid_count}/{total_count} 個の値が有効",
                )
                ...
                if isinstance(arr, np.ndarray):
                    ...
                    if hasattr(arr, "shape") and len(arr.shape) > 1:
                        ...
                        total_elements = (
                            ...
                            if hasattr(arr.shape, "__len__") and len(arr.shape) > 1
                            else arr.shape[0]
                        )
                        valid_counts.append(valid_count)
                    else:
            ...
            return (
                TestResultCategory.SUCCESS,
                result,
                f"{len(valid_counts)}/{total_arrays} 配列に有効値",
            )
...
def execute_single_indicator_test(
    df: DataFrame, indicator_name: str
) -> IndicatorTestResult:
    """単一インジケーターの全面テスト実行"""
    ...
    result = IndicatorTestResult(
        indicator_name=indicator_name, category=TestResultCategory.SUCCESS
    )
    ...
    # 2. 計算テスト
    calc_category, calc_result, calc_details = test_indicator_calculation(
        df, indicator_name
    )
...
def execute_batch_tests(
    indicators: List[str], max_workers: int = 4
    ...
    logger.info(
        f"{len(indicators)}個のインジケーターを{max_workers}ワーカーでテスト実行開始"
    )
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        future_to_indicator = {
            executor.submit(execute_single_indicator_test, df, indicator): indicator
...
def generate_test_report(
    report: ComprehensiveTestReport, output_path: Optional[str] = None
) -> str:
    """テストレポート生成"""
    ...
    report_lines.extend([f"", f"【カテゴリ別詳細】"])

    for category in TestResultCategory:
        results = report.results_by_category.get(category, [])
        if results:
            report_lines.extend([f"", f"{category.value} ({len(results)}個):"])
            ...
            for result in sorted_results:
                ...
                if (
                    result.valid_values_count is not None
                    and result.total_values_count is not None
                ):
...
class TestRegimeAwareIntegration:
    ...
    def test_regime_detector_with_different_regimes(self, regime_detector):
        ...
        for regime_name, data in regime_data.items():
            ...
            with patch.object(
                regime_detector, "detect_regimes", return_value=expected_regimes
            ):
    ...
    def test_ga_performance_comparison_with_regime_adaptation(
        ...
        mock_regime_detector_config,
        mock_ga_config_with_regime,
        mock_ga_config_without_regime,
    ):
        ...
        # モックデータを準備
        test_data = create_regime_specific_test_data(100)[
            "trend"
        ]  # トレンドデータを使用
        ...
        with patch.object(
            regime_detector,
            "detect_regimes",
            return_value=np.array([0] * 50 + [1] * 30 + [2] * 20),
        ):
            ...
            # レジーム適応有効での評価
            evaluator_with_regime = IndividualEvaluator(
                mock_backtest_service, regime_detector
            )
            ...
            # 両方の設定で評価
            fitness_with_regime = evaluator_with_regime.evaluate_individual(
                individual, mock_ga_config_with_regime
            )
            fitness_without_regime = evaluator_without_regime.evaluate_individual(
                individual, mock_ga_config_without_regime
            )
    ...
    def test_regime_transition_impact_on_performance(
        self, regime_detector, mock_backtest_service
    ):
        ...
        with patch.object(
            regime_detector, "detect_regimes", return_value=transition_regimes
        ):
            # レジーム検知
    ...
    def test_regime_detection_failure_fallback(
    ...
    def test_sharpe_ratio_drawdown_comparison_across_regimes(
        self, regime_detector, mock_backtest_service
    ):
        ...
        for regime_name, data in regime_data.items():
            ...
            with patch.object(
                regime_detector, "detect_regimes", return_value=mock_regimes
            ):
                regimes = regime_detector.detect_regimes(data)
    ...
    try:
        ...
        for indicator in excluded_indicators:
            if indicator in indicators:
                print(
                    f"WARNING: {indicator}インジケーターがまだテスト対象に含まれています"
                )
                all_excluded = False
        if all_excluded:
            print(
                "SUCCESS: STC、AOBV、HWC、RVGI、PPO、PVO、KVOインジケーターが完全に除外されました"
            )
        ...
        # 全体成功率チェック
        success_count = len(
            report.results_by_category.get(TestResultCategory.SUCCESS, [])
        )
