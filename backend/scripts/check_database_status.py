        ...
        try:
            ...
            for symbol in symbols[:5]:  # 最初の5つのシンボルを詳細確認
                ...
                for timeframe in timeframes[:3]:  # 最初の3つの時間軸を確認
                    ...
                    if count > 0:
                        date_range = ohlcv_repo.get_date_range(symbol, timeframe)
                        ...
                        # 最新データのサンプル表示
                        latest_data = ohlcv_repo.get_ohlcv_data(symbol, timeframe, limit=1)
                        if latest_data:
                            data = latest_data[0]
                            print(f"      最新価格: O:{data.open:.2f} H:{data.high:.2f} L:{data.low:.2f} C:{data.close:.2f}")
                ...
                try:
                    ...
                    if oi_data:
                        ...
                        if all_oi_data:
                            ...
                            print(f"    期間: {oldest_oi.strftime('%Y-%m-%d')} ～ {latest_oi.strftime('%Y-%m-%d')}")
            ...
            for symbol in symbols[:5]:
                try:
                    ...
                    if fr_data:
                        ...
                        if all_fr_data:
                            oldest_fr = min(data.funding_timestamp for data in all_fr_data)
                            latest_fr = max(data.funding_timestamp for data in all_fr_data)
                            
                            print(f"💸 {symbol}:")
                            print(f"    データ件数: {len(all_fr_data):,} 件")
                            print(f"    期間: {oldest_fr.strftime('%Y-%m-%d')} ～ {latest_fr.strftime('%Y-%m-%d')}")
                            print(f"    最新FR: {fr_data[0].funding_rate:.6f} ({fr_data[0].funding_rate*100:.4f}%)")
                except Exception as e:
            ...
            for symbol in symbols:
                has_ohlcv = ohlcv_repo.get_data_count(symbol, "1d") > 30  # 30日以上のデータ
                has_oi = symbol in oi_symbols
                has_fr = symbol in fr_symbols
                
                if has_ohlcv and (has_oi or has_fr):  # OHLCVと少なくともOIまたはFRがある
            ...
            for data in complete_symbols:
                ...
                print(f"  📊 {data['symbol']}: OHLCV({data['ohlcv_count']}件) OI({oi_status}) FR({fr_status})")
