...
def run_multiple_regime_tests(num_runs: int = 5) -> Dict[str, Any]:
    ...
    for regime_enabled in [True, False]:
        for run in range(num_runs):
            ...
            start_date = (base_start + pd.Timedelta(days=start_offset)).strftime(
                "%Y-%m-%d"
            )
            end_date = (pd.to_datetime(start_date) + pd.Timedelta(days=365)).strftime(
                "%Y-%m-%d"
            )
    ...
    csv_path = os.path.join(
        os.path.dirname(__file__), "..", "multiple_regime_tests_results.csv"
    )
    ...
    return {"results": results, "summary": summary, "csv_path": csv_path}
