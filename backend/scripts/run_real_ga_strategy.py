...
def test_data_integration():
    ...
    try:
        ...
        if "open_interest_stats" in summary:
            ...
            print(
                f"  OI統計: 平均={oi_stats['average']:,.0f}, 範囲={oi_stats['min']:,.0f}～{oi_stats['max']:,.0f}"
            )

        if "funding_rate_stats" in summary:
            fr_stats = summary["funding_rate_stats"]
            print(
                f"  FR統計: 平均={fr_stats['average']:.6f}, 範囲={fr_stats['min']:.6f}～{fr_stats['max']:.6f}"
            )
...
def generate_real_strategies():
    ...
    try:
        ...
        ga_config = GAConfig(
            max_indicators=4, min_indicators=2, max_conditions=3, min_conditions=1
        )
        ...
        print(
            f"  有効戦略率: {valid_count}/{len(strategies)} ({valid_count/len(strategies)*100:.1f}%)"
        )
        print(
            f"  OI/FR活用率: {oi_fr_usage_count}/{len(strategies)} ({oi_fr_usage_count/len(strategies)*100:.1f}%)"
        )
