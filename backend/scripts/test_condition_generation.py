...
def test_condition_generation():
    ...
    print("📊 設定値:")
    print(f"  数値閾値使用確率: {ga_config.numeric_threshold_probability:.1%}")
    print(f"  最小互換性スコア: {ga_config.min_compatibility_score}")
    print(f"  厳密互換性スコア: {ga_config.strict_compatibility_score}")
    print()
        ...
        try:
            ...
            if isinstance(right, (int, float)):
                ...
                if compatibility < 0.3:
                    scale_mismatches += 1
                    print(
                        f"  {i+1:2d}. {left} {op} {right} (互換性: {compatibility:.2f}) ❌ スケール不一致"
                    )
                elif compatibility < 0.8:
                    print(
                        f"  {i+1:2d}. {left} {op} {right} (互換性: {compatibility:.2f}) ⚠️ 低い互換性"
                    )
                else:
                    print(
                        f"  {i+1:2d}. {left} {op} {right} (互換性: {compatibility:.2f}) ✅ 高い互換性"
                    )
    ...
    print(
        f"  数値条件: {numerical_conditions} ({numerical_conditions/total_conditions:.1%})"
    )
    print(
        f"  スケール不一致: {scale_mismatches} ({scale_mismatches/total_conditions:.1%})"
    )
    print()
    ...
    expected_numerical_ratio = ga_config.numeric_threshold_probability
    actual_numerical_ratio = (
    ...
    if abs(actual_numerical_ratio - expected_numerical_ratio) > 0.2:
        ...
        print(
            f"     期待値: {expected_numerical_ratio:.1%}, 実際: {actual_numerical_ratio:.1%}"
        )
    else:
