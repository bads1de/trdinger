...
オートストラテジー機能（10個の厳選指標対応）の完全動作を確認します。
"""
...
def test_frontend_backend_integration():
    ...
    frontend_config = {
        ...
        "base_config": {
            ...
            "strategy_config": {"strategy_type": "", "parameters": {}},
        },
        "ga_config": {
    ...
    print(
        f"計算量: {frontend_config['ga_config']['population_size'] * frontend_config['ga_config']['generations']}"
    )
...
def test_all_indicators_compatibility():
    ...
    for category, indicators in categories.items():
        ...
        for indicator in indicators:
            try:
                ...
                indicator_gene = IndicatorGene(
                    type=indicator, parameters=params, enabled=True
                )
                ...
                print(
                    f"  ✅ {indicator:12}: パラメータ={len(params)}, 条件={len(entry_conditions)}+{len(exit_conditions)}"
                )
                category_success += 1
                total_success += 1
        ...
        print(
            f"  カテゴリ成功率: {category_success}/{len(indicators)} ({category_success/len(indicators)*100:.1f}%)"
        )
...
def test_performance_improvements():
    ...
    print_section("設定比較")
    print(
        f"{'設定名':<10} {'個体数':<6} {'世代数':<6} {'計算量':<8} {'削減率':<8} {'予想時間':<10}"
    )
    print("-" * 60)
    ...
    for name, config in configs:
        ...
        reduction = (
            (base_calculations - calculations) / base_calculations * 100
            if calculations != base_calculations
            else 0
        )
        estimated_time = calculations * 0.1 / 60  # 1評価0.1秒、分単位

        print(
            f"{name:<10} {config['population_size']:<6} {config['generations']:<6} {calculations:<8} {reduction:>6.1f}% {estimated_time:>8.1f}分"
        )
...
def test_strategy_quality():
    ...
    for strategy in strategies:
        ...
        # 条件の複雑性
        total_conditions = len(strategy.entry_conditions) + len(
    ...
    return len(indicator_diversity), sum(condition_complexity) / len(
        condition_complexity
    )
...
def save_final_test_results(results: Dict[str, Any]):
    ...
    output_path = os.path.join(
        os.path.dirname(__file__), "final_integration_test_results.json"
    )
...
def main():
    ...
    try:
        ...
        print(
            f"✅ フロントエンド・バックエンド統合: {'成功' if results['integration_test'] else '失敗'}"
        )
        print(
            f"✅ 全58指標互換性: {'成功' if results['indicators_compatibility'] else '失敗'}"
        )
        print(
            f"✅ パフォーマンス改善: 指標カバー率 {results['performance']['coverage_rate']:.1f}%"
        )
        print(
            f"✅ 戦略品質向上: 指標多様性 {results['quality']['indicator_diversity']}種類"
        )
