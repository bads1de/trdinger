...
class SimpleGeneGenerator:
    ...
    def __init__(self):
        ...
        self.available_data_sources = ["close", "open", "high", "low", "volume", "OpenInterest", "FundingRate"]
    ...
    def generate_random_gene(self) -> SimpleStrategyGene:
        ...
        for _ in range(num_indicators):
            ...
            indicators.append(SimpleIndicatorGene(
                type=indicator_type,
        ...
        return SimpleStrategyGene(
            ...
            risk_management={"stop_loss": random.uniform(0.02, 0.05), "take_profit": random.uniform(0.05, 0.15)}
        )
    
    def _generate_conditions(self, indicators: List[SimpleIndicatorGene], count: int) -> List[SimpleCondition]:
        """条件を生成"""
        conditions = []
        ...
        for _ in range(count):
            ...
            if random.random() < 0.4:  # 40%の確率で数値
                if "FundingRate" in left_operand:
                    right_operand = random.choice([0.0001, 0.0005, 0.001, -0.0001, -0.0005, -0.001])
                elif "OpenInterest" in left_operand:
                    right_operand = random.choice([1000000, 5000000, 10000000, 50000000])
...
def test_real_data_integration():
    ...
    try:
        ...
        print(f"  FR範囲: {df['FundingRate'].min():.6f} ～ {df['FundingRate'].max():.6f}")
...
def generate_and_evaluate_strategies():
    ...
    try:
        ...
        for i in range(10):
            ...
            print(f"    指標: {[f'{ind.type}({ind.parameters})' for ind in strategy.indicators]}")
            ...
            print(f"    📋 エントリー: {[f'{c.left_operand} {c.operator} {c.right_operand}' for c in strategy.entry_conditions]}")
        ...
        for i, strategy in enumerate(strategies):
            ...
            print(f"  戦略{i+1}: リターン={result['total_return']:.1f}% シャープ={result['sharpe_ratio']:.2f} "
...
def analyze_final_results(results):
    ...
    if oi_fr_strategies and non_oi_fr_strategies:
        ...
        oi_fr_avg_fitness = sum(r['fitness'] for r in oi_fr_strategies) / len(oi_fr_strategies)
        non_oi_fr_avg_fitness = sum(r['fitness'] for r in non_oi_fr_strategies) / len(non_oi_fr_strategies)
