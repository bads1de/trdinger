    ...
    try:
        ...
        class TestRSIStrategy(Strategy):
            def init(self):
                ...
                rsi_result = MomentumAdapter.rsi(close_prices, period=14)
                ...
                if valid_rsi:
                    ...
                    print(f"  📊 RSI統計:")
                    print(f"    売られすぎ (RSI < 30): {oversold_count}/{len(valid_rsi)} ({oversold_count/len(valid_rsi)*100:.1f}%)")
                    print(f"    買われすぎ (RSI > 70): {overbought_count}/{len(valid_rsi)} ({overbought_count/len(valid_rsi)*100:.1f}%)")
                ...
                if current_index < len(self.rsi):
                    ...
                    if not pd.isna(current_rsi) and current_rsi < 30:
                        if not self.position:
                            # 小さなポジションサイズで取引
                            size = 0.01  # 1%のポジション
                            print(f"  🟢 エントリーシグナル: RSI={current_rsi:.2f} < 30, サイズ={size}")
                            self.buy(size=size)
                    
                    # エグジット条件: RSI > 70 (買われすぎ)
                    elif not pd.isna(current_rsi) and current_rsi > 70:
                        if self.position:
                            print(f"  🔴 エグジットシグナル: RSI={current_rsi:.2f} > 70")
                            self.sell()
        
        # バックテストを実行（十分な資金で）
        print("⚡ 手動バックテスト実行中（十分な資金）...")
        bt = Backtest(data, TestRSIStrategy, cash=10000000, commission=0.001)  # 1000万円の資金
        result = bt.run()
        ...
        if len(trades) > 0:
            ...
            for i, trade in trades.head(5).iterrows():  # 最初の5件を表示
                print(f"  取引 {i+1}:")
                print(f"    エントリー: {trade['EntryTime']} @ {trade['EntryPrice']:.2f}")
        ...
        print(f"  総リターン: {generated_result['performance_metrics']['total_return']:.4f}")
        print(f"  最終資産: {generated_result['performance_metrics']['equity_final']:,.0f}")
        print(f"  勝率: {generated_result['performance_metrics']['win_rate']}")
        print(f"  最大ドローダウン: {generated_result['performance_metrics']['max_drawdown']:.4f}")
        ...
        if trade_history:
            ...
            for i, trade in enumerate(trade_history[:5], 1):  # 最初の5件を表示
                print(f"  取引 {i}:")
                print(f"    エントリー: {trade.get('entry_time')} @ {trade.get('entry_price')}")
                print(f"    エグジット: {trade.get('exit_time')} @ {trade.get('exit_price')}")
        ...
        if manual_trades > 0 and generated_trades == 0:
            print("  ❌ 手動実装では取引が発生するが、GENERATED_TEST戦略では取引が発生しない")
