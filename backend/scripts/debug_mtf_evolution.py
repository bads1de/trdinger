...
def print_condition_structure(cond, indent=0):
    ...
    if isinstance(cond, ConditionGroup):
        ...
        print(f"{prefix}[COND] {cond.left_operand} {cond.operator} {cond.right_operand} (Dir: {direction})")
    else:
...
def debug_mtf_flow():
    ...
    try:
        ...
        if mtf_condition:
            ...
            logger.warning("WARNING: MTF構造の条件が生成されませんでした（ランダム要素のため再実行してください）")
            # 強制的にテスト用MTF条件を作成
    ...
    # 構造が維持されているか確認
    if isinstance(child1, ConditionGroup) and len(child1.conditions) == len(parent1.conditions):
