...
def clear_all_ohlcv_data() -> int:
    ...
    if SessionLocal is None or OHLCVRepository is None:
        logger.error("エラー: データベースコンポーネントが利用できません。")
        return 0
    ...
    try:
        ...
        # ユーザー確認
        response = input(
            f"本当に全ての{count_before}件のOHLCVデータを削除しますか？ (yes/no): "
        )
...
def clear_ohlcv_data_by_symbol(symbol: str) -> int:
    ...
    try:
        ...
        # ユーザー確認
        response = input(
            f"本当にシンボル '{symbol}' の{count_before}件のデータを削除しますか？ (yes/no): "
        )
        if response.lower() != "yes":
            logger.info("削除をキャンセルしました")
            return 0
...
def clear_unsupported_symbols() -> int:
    ...
    try:
        ...
        # ユーザー確認
        response = input(
            f"本当にサポートされていない{len(unsupported_symbols)}個のシンボル（計{total_deleted}件）を削除しますか？ (yes/no): "
        )
        if response.lower() != "yes":
            logger.info("削除をキャンセルしました")
            return 0
        ...
        logger.info(
            f"✅ 合計{actual_deleted}件のサポートされていないシンボルデータを削除しました"
        )
...
def show_current_data_status():
    ...
    try:
        ...
        supported_symbols = [
            s for s in available_symbols if s in MarketDataConfig.SUPPORTED_SYMBOLS
        ]
        unsupported_symbols = [
            s for s in available_symbols if s not in MarketDataConfig.SUPPORTED_SYMBOLS
        ]
...
def clear_ohlcv_data_by_timeframe(timeframe: str) -> int:
    """
    ...
    try:
        ...
        count_before = (
            db.query(OHLCVData).filter(OHLCVData.timeframe == timeframe).count()
        )
        logger.info(f"削除前の{timeframe}データ件数: {count_before}")

        if count_before == 0:
            logger.info(f"時間足 '{timeframe}' のデータがありません")
            return 0

        # ユーザー確認
        response = input(
            f"本当に時間足 '{timeframe}' の{count_before}件のデータを削除しますか？ (yes/no): "
        )
        if response.lower() != "yes":
            logger.info("削除をキャンセルしました")
            return 0
        ...
        # データ削除実行
        deleted_count = repo.clear_ohlcv_data_by_timeframe(timeframe)
        logger.info(
            f"✅ 時間足 '{timeframe}' の{deleted_count}件のデータを削除しました"
        )

        return deleted_count

    except Exception as e:
        logger.error(f"❌ 時間足 '{timeframe}' のデータ削除エラー: {e}")
        raise
    finally:
        db.close()
