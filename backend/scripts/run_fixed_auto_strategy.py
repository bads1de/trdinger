    ...
    try:
        ...
        while time.time() - start_time < max_wait:
            ...
            if progress:
                ...
                total_gen = getattr(
                    progress, "total_generations", ga_config.generations
                )
                ...
                if progress.status == "completed":
                    ...
                    if result:
                        ...
                        print(
                            f"  エントリー条件数: {len(best_strategy.entry_conditions)}"
                        )
                        print(
                            f"  エグジット条件数: {len(best_strategy.exit_conditions)}"
                        )
                        ...
                        for i, condition in enumerate(
                            best_strategy.entry_conditions, 1
                        ):
                            print(
                                f"    {i}. {condition.left_operand} {condition.operator} {condition.right_operand}"
                            )
            ...
            if final_progress:
                ...
                if (
                    hasattr(final_progress, "error_message")
                    and final_progress.error_message
                ):
        ...
        try:
            ...
            for result in recent_results:
                if (
                    ...
                    and "FIXED_AUTO_STRATEGY" in result.strategy_name
                ):
                    ...
                    if result.performance_metrics:
                        ...
                        print(
                            f"      勝率: {win_rate:.2%}"
                            if win_rate and not str(win_rate) == "nan"
                            else "      勝率: N/A"
                        )
                        ...
                        if total_trades > 0:
                            ...
                            if result.trade_history:
                                ...
                                for i, trade in enumerate(result.trade_history[:3], 1):
                                    ...
                                    pnl = trade.get("pnl", 0)
                                    print(
                                        f"      {i}. {entry_time} - {exit_time}: {pnl:.2f}"
                                    )
                        else:
