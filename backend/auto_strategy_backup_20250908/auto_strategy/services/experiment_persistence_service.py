...
class ExperimentPersistenceService:
    ...
    def get_experiment_results(self, experiment_id: str) -> Optional[Dict[str, Any]]:
        ...
        @safe_operation(context="実験結果取得", is_api_call=False, default_return=None)
        def _get_experiment_results():
            with self.db_session_factory() as db:
                backtest_result_repo = BacktestResultRepository(db)

                # バックテスト結果を取得（experiment_idから）
                backtest_results = backtest_result_repo.get_backtest_results_by_experiment(
                    experiment_id
                )
                ...
                # パフォーマンスメトリクスを整形
                metrics = latest_result.performance_metrics or {}
    ...
    def get_experiment_status(self, experiment_id: str) -> Optional[Dict[str, Any]]:
        ...
        @safe_operation(context="実験ステータス取得", is_api_call=False, default_return=None)
        def _get_experiment_status():
            with self.db_session_factory() as db:
                ...
                if status == "running":
                    # 実行中なら進捗を推定
                    # TODO: 実際のGA進捗取得ロジックを実装
                    progress = min(50.0, 100.0)  # 仮の進捗
                    current_generation = max(1, int(progress / 100 * 10))  # 仮の世代数
                    eta_minutes = max(1, int((100 - progress) / 10))  # 仮の推定時間
                elif status in ["completed", "failed", "stopped"]:
                    progress = 100.0
                    current_generation = 10  # 仮の最終世代数
                    eta_minutes = 0
                ...
                return {
                    ...
                    "created_at": experiment_info.get("created_at").isoformat() if experiment_info.get("created_at") else None,
                    "completed_at": experiment_info.get("completed_at").isoformat() if experiment_info.get("completed_at") else None,
                }
